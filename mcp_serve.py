"""
Iterativ MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Iterativ-specific extra)

Usage:
    iterativ mcp serve
    iterativ mcp serve --verbose
...
logger = logging.getLogger("iterativ.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ITERATIV_HOME."""
    try:
        from iterativ_constants import get_iterativ_home
        return get_iterativ_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ITERATIV_HOME", Path.home() / ".iterativ")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from iterativ_constants import get_iterativ_home
        directory_file = get_iterativ_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ITERATIV_HOME", Path.home() / ".iterativ")
...
class EventBridge:
    ...
    This is the Iterativ equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from iterativ_constants import get_iterativ_home
            db_file = get_iterativ_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ITERATIV_HOME", Path.home() / ".iterativ")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Iterativ MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "iterativ",
        instructions=(
            "Iterativ Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Iterativ MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
