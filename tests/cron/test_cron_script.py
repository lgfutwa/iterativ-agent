...
- Path resolution (absolute, relative to ITERATIV_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ITERATIV_HOME."""
    iterativ_home = tmp_path / ".iterativ"
    iterativ_home.mkdir()
    (iterativ_home / "cron").mkdir()
    (iterativ_home / "cron" / "output").mkdir()
    (iterativ_home / "scripts").mkdir()
    monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ITERATIV_DIR", iterativ_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", iterativ_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", iterativ_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", iterativ_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.iterativ/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ITERATIV_SESSION_PLATFORM") is None
        assert os.environ.get("ITERATIV_SESSION_CHAT_ID") is None
        assert os.environ.get("ITERATIV_SESSION_CHAT_NAME") is None
