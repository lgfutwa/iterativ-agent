"""Tests for ACP Registry metadata shipped with Iterativ."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "iterativ-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Iterativ Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/IterativKnowledge/iterativ-agent"
    assert data["website"].startswith("https://iterativ-agent.nousresearch.com/")
    assert data["authors"] == ["Iterativ Knowledge"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"iterativ-agent[acp]=={data['version']}"
    assert uvx["args"] == ["iterativ-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"iterativ-agent[acp]=={_pyproject_version()}"
    )
