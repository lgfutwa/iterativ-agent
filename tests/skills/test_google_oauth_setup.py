...
class TestIterativConstantsFallback:
    """Tests for _iterativ_home.py fallback when iterativ_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_iterativ_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _iterativ_home.py with iterativ_constants blocked."""
        monkeypatch.setitem(sys.modules, "iterativ_constants", None)
        spec = importlib.util.spec_from_file_location("_iterativ_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_iterativ_home_env_var(self, monkeypatch, tmp_path):
        """When iterativ_constants is missing, ITERATIV_HOME comes from env var."""
        monkeypatch.setenv("ITERATIV_HOME", str(tmp_path / "custom-iterativ"))
        module = self._load_helper(monkeypatch)
        assert module.get_iterativ_home() == tmp_path / "custom-iterativ"

    def test_fallback_defaults_to_dot_iterativ(self, monkeypatch):
        """When iterativ_constants is missing and ITERATIV_HOME unset, default to ~/.iterativ."""
        monkeypatch.delenv("ITERATIV_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_iterativ_home() == Path.home() / ".iterativ"

    def test_fallback_ignores_empty_iterativ_home(self, monkeypatch):
        """Empty/whitespace ITERATIV_HOME is treated as unset."""
        monkeypatch.setenv("ITERATIV_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_iterativ_home() == Path.home() / ".iterativ"

    def test_fallback_display_iterativ_home_shortens_path(self, monkeypatch):
        """Fallback display_iterativ_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ITERATIV_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_iterativ_home() == "~/.iterativ"

    def test_fallback_display_iterativ_home_profile_path(self, monkeypatch):
        """Fallback display_iterativ_home() handles profile paths under ~/."""
        monkeypatch.setenv("ITERATIV_HOME", str(Path.home() / ".iterativ/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_iterativ_home() == "~/.iterativ/profiles/coder"

    def test_fallback_display_iterativ_home_custom_path(self, monkeypatch):
        """Fallback display_iterativ_home() returns full path for non-home locations."""
        monkeypatch.setenv("ITERATIV_HOME", "/opt/iterativ-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_iterativ_home() == "/opt/iterativ-custom"

    def test_delegates_to_iterativ_constants_when_available(self):
        """When iterativ_constants IS importable, _iterativ_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_iterativ_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import iterativ_constants
        assert module.get_iterativ_home is iterativ_constants.get_iterativ_home
        assert module.display_iterativ_home is iterativ_constants.display_iterativ_home
