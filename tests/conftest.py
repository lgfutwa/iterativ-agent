"""Shared fixtures for the iterativ-agent test suite.
...
2. **Isolated ITERATIV_HOME.** ITERATIV_HOME points to a per-test tempdir so
   code reading ``~/.iterativ/*`` via ``get_iterativ_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".iterativ"`` instead of the canonical
   ``get_iterativ_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ITERATIV_SESSION_* inheritance** — the agent's current gateway
...
# ITERATIV_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ITERATIV_BEHAVIORAL_VARS = frozenset({
    "ITERATIV_YOLO_MODE",
    "ITERATIV_INTERACTIVE",
    "ITERATIV_QUIET",
    "ITERATIV_TOOL_PROGRESS",
    "ITERATIV_TOOL_PROGRESS_MODE",
    "ITERATIV_MAX_ITERATIONS",
    "ITERATIV_SESSION_PLATFORM",
    "ITERATIV_SESSION_CHAT_ID",
    "ITERATIV_SESSION_CHAT_NAME",
    "ITERATIV_SESSION_THREAD_ID",
    "ITERATIV_SESSION_SOURCE",
    "ITERATIV_SESSION_KEY",
    "ITERATIV_GATEWAY_SESSION",
    "ITERATIV_PLATFORM",
    "ITERATIV_MODEL",
    "ITERATIV_INFERENCE_MODEL",
    "ITERATIV_INFERENCE_PROVIDER",
    "ITERATIV_TUI_PROVIDER",
    "ITERATIV_MANAGED",
    "ITERATIV_DEV",
    "ITERATIV_CONTAINER",
    "ITERATIV_EPHEMERAL_SYSTEM_PROMPT",
    "ITERATIV_TIMEZONE",
    "ITERATIV_REDACT_SECRETS",
    "ITERATIV_BACKGROUND_NOTIFICATIONS",
    "ITERATIV_EXEC_ASK",
    "ITERATIV_HOME_MODE",
    "ITERATIV_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.iterativ/kanban.db instead of the per-test ITERATIV_HOME.
    "ITERATIV_KANBAN_DB",
    "ITERATIV_KANBAN_BOARD",
    "ITERATIV_KANBAN_HOME",
    "ITERATIV_KANBAN_WORKSPACES_ROOT",
    "ITERATIV_KANBAN_LOGS_ROOT",
    "ITERATIV_KANBAN_TASK",
    "ITERATIV_KANBAN_WORKSPACE",
    "ITERATIV_KANBAN_RUN_ID",
    "ITERATIV_KANBAN_CLAIM_LOCK",
    "ITERATIV_KANBAN_DISPATCH_IN_GATEWAY",
    "ITERATIV_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ITERATIV_HOME to per-test tempdirs so code that
    reads ``~/.iterativ/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ITERATIV_* vars that could change test semantics.
    for name in _ITERATIV_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ITERATIV_HOME to a per-test tempdir. Code that reads
    #    ``~/.iterativ/*`` via ``get_iterativ_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.iterativ/*`` via
    #    ``Path.home() / ".iterativ"`` instead of ``get_iterativ_home()``
    #    is a bug to fix at the callsite.
    fake_iterativ_home = tmp_path / "iterativ_test"
    fake_iterativ_home.mkdir()
    (fake_iterativ_home / "sessions").mkdir()
    (fake_iterativ_home / "cron").mkdir()
    (fake_iterativ_home / "memories").mkdir()
    (fake_iterativ_home / "skills").mkdir()
    monkeypatch.setenv("ITERATIV_HOME", str(fake_iterativ_home))
    ...
    #    ~/.iterativ/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import iterativ_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_iterativ_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "iterativ_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal iterativ config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``iterativ-gateway`` process
...
# correlated with a ``tests/iterativ_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> iterativ-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart iterativ-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting iterativ/python patterns are also blocked.
    """
    ...
    def _matches_iterativ_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ITERATIV_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_iterativ_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch iterativ-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m iterativ_cli.main".
                if (
                    "iterativ" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live iterativ-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting iterativ/python could hit the live gateway. "
