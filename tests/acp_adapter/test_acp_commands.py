...
from acp_adapter.server import IterativACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["iterativ-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "iterativ_cli.config",
        mod("iterativ_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
