...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "iterativ"
        assert config.workspace_id == "iterativ"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.iterativ/honcho.json
        monkeypatch.setenv("ITERATIV_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"iterativ": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"iterativ": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"iterativ": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="iterativ-agent"
        ):
            result = config.resolve_session_name("/home/user/iterativ-agent/subdir")
        assert result == "iterativ-agent"
...
class TestResolveConfigPath:
    def test_prefers_iterativ_home_when_exists(self, tmp_path):
        iterativ_home = tmp_path / "iterativ"
        iterativ_home.mkdir()
        local_cfg = iterativ_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"ITERATIV_HOME": str(iterativ_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ITERATIV_HOME points at ~/.iterativ/profiles/<name>, so
        # _get_default_iterativ_home() must resolve back to ~/.iterativ — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".iterativ"
        ...
        assert _get_default_iterativ_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_iterativ_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("ITERATIV_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".iterativ".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".iterativ"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_iterativ(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ITERATIV_HONCHO_HOST", None)
            os.environ.pop("ITERATIV_HOME", None)
            assert resolve_active_host() == "iterativ"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ITERATIV_HONCHO_HOST": "iterativ.coder"}):
            assert resolve_active_host() == "iterativ.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ITERATIV_HONCHO_HOST", None)
            with patch("iterativ_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "iterativ.coder"

    def test_default_profile_returns_iterativ(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ITERATIV_HONCHO_HOST", None)
            with patch("iterativ_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "iterativ"

    def test_custom_profile_returns_iterativ(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ITERATIV_HONCHO_HOST", None)
            with patch("iterativ_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "iterativ"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ITERATIV_HONCHO_HOST", None)
            # Temporarily remove iterativ_cli.profiles to simulate import failure
            saved = sys.modules.get("iterativ_cli.profiles")
            sys.modules["iterativ_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "iterativ"
            finally:
                if saved is not None:
                    sys.modules["iterativ_cli.profiles"] = saved
                else:
                    sys.modules.pop("iterativ_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="iterativ.coder")
        assert config.host == "iterativ.coder"
        assert config.workspace_id == "iterativ"  # shared workspace
        assert config.ai_peer == "iterativ.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="iterativ")
        assert config.host == "iterativ"
        assert config.workspace_id == "iterativ"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "iterativ": {"aiPeer": "iterativ", "peerName": "alice"},
                "iterativ.coder": {
                    "aiPeer": "iterativ.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="iterativ.coder", config_path=config_file,
        )
        assert config.host == "iterativ.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "iterativ.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="iterativ.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "iterativ.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"iterativ": {"enabled": True, "aiPeer": "iterativ"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"iterativ": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_iterativ_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="iterativ",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("iterativ_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("iterativ_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_iterativ_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="iterativ",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("iterativ_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"iterativ": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
