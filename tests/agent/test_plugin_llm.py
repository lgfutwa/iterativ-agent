...
""",
        ...
        from iterativ_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        (iterativ_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
        from iterativ_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from iterativ_cli.plugins import PluginContext, PluginManifest, PluginManager
