...
def test_iterativ_client_tag_includes_current_version():
    """The client tag must reflect iterativ_cli.__version__ verbatim."""
    from iterativ_cli import __version__
    from agent.portal_tags import iterativ_client_tag

    assert iterativ_client_tag() == f"client=iterativ-client-v{__version__}"


def test_iterativ_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import iterativ_client_tag

    tag = iterativ_client_tag()
    assert tag.startswith("client=iterativ-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import iterativ_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=iterativ-agent" in tags
    assert iterativ_client_tag() in tags
    assert len(tags) == 2
