...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Iterativ passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ITERATIV_HOME", "/users/alice/.iterativ/profiles/backend-worker")
        monkeypatch.setenv("ITERATIV_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ITERATIV_KANBAN_DB",
            "/users/alice/.iterativ/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.iterativ/kanban/boards/smoke"]'
            in cmd
        )
