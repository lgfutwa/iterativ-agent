...
class TestToolsetConsistency:
    ...
    def test_iterativ_platforms_share_core_tools(self):
        """All iterativ-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on iterativ-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["iterativ-cli", "iterativ-telegram", "iterativ-discord", "iterativ-whatsapp", "iterativ-slack", "iterativ-signal", "iterativ-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_iterativ_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("iterativ-whatsapp")

    def test_iterativ_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("iterativ-api-server")
