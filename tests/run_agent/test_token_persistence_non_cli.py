...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    iterativ_state = ModuleType("iterativ_state")
    iterativ_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "iterativ_state", iterativ_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Iterativ"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Iterativ"
