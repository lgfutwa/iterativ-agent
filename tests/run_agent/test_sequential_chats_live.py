...
AlexKucera reported on Discord (2026-04-16) that after ``iterativ update`` pulled
...
Opt-in — not part of default CI:
    ITERATIV_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.iterativ/.env).
"""
...
# Load ~/.iterativ/.env so live runs pick up OPENROUTER_API_KEY without
# needing the runner to shell-source it first. Silent if the file is absent.
def _load_user_env() -> None:
    env_file = Path.home() / ".iterativ" / ".env"
...
LIVE = os.environ.get("ITERATIV_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set ITERATIV_LIVE_TESTS=1"),
