...
See: https://github.com/IterativKnowledge/iterativ-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for iterativ_constants.get_subprocess_home()."""

    def test_returns_none_when_iterativ_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        profile_home = iterativ_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
        from iterativ_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".iterativ" / "profiles"
        ...
        monkeypatch.setenv("ITERATIV_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ITERATIV_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from iterativ_constants import (
            get_iterativ_home,
            reset_iterativ_home_override,
            set_iterativ_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_iterativ_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_iterativ_home_override(profile)
        try:
            assert get_iterativ_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_iterativ_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_iterativ_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(iterativ_home / "home")
    ...
    def test_no_injection_when_iterativ_home_unset(self, monkeypatch):
        monkeypatch.delenv("ITERATIV_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["ITERATIV_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from iterativ_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from iterativ_cli.profiles import create_profile
