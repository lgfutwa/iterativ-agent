"""Tests for iterativ_constants module."""
...
from iterativ_constants import (
    ...
    get_default_iterativ_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultIterativRoot:
    """Tests for get_default_iterativ_root() — Docker/custom deployment awareness."""

    def test_no_iterativ_home_returns_native(self, tmp_path, monkeypatch):
        """When ITERATIV_HOME is not set, returns ~/.iterativ."""
        monkeypatch.delenv("ITERATIV_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_iterativ_root() == tmp_path / ".iterativ"

    def test_iterativ_home_is_native(self, tmp_path, monkeypatch):
        """When ITERATIV_HOME = ~/.iterativ, returns ~/.iterativ."""
        native = tmp_path / ".iterativ"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(native))
        assert get_default_iterativ_root() == native

    def test_iterativ_home_is_profile(self, tmp_path, monkeypatch):
        """When ITERATIV_HOME is a profile under ~/.iterativ, returns ~/.iterativ."""
        native = tmp_path / ".iterativ"
    ...
    def test_iterativ_home_is_docker(self, tmp_path, monkeypatch):
        """When ITERATIV_HOME points outside ~/.iterativ (Docker), returns ITERATIV_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(docker_home))
        assert get_default_iterativ_root() == docker_home

    def test_iterativ_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ITERATIV_HOME outside ~/.iterativ is treated as the root."""
        custom = tmp_path / "my-iterativ-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(custom))
        assert get_default_iterativ_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ITERATIV_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(profile))
        assert get_default_iterativ_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(iterativ_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(iterativ_constants, "_container_detected", True)
        assert is_container() is True
