"""
Tests for mcp_serve — Iterativ MCP server.
...
def _isolate_iterativ_home(tmp_path, monkeypatch):
    """Redirect ITERATIV_HOME to a temp directory."""
    monkeypatch.setenv("ITERATIV_HOME", str(tmp_path))
    try:
        import iterativ_constants
        monkeypatch.setattr(iterativ_constants, "get_iterativ_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking iterativ_state schema."""
