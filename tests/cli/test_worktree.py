...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("iterativ/iterativ-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned iterativ/* and pr-* branches."""

    def test_prunes_orphaned_iterativ_branch(self, git_repo):
        """iterativ/iterativ-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "iterativ/iterativ-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "iterativ/iterativ-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "iterativ/iterativ-deadbeef" in result.stdout
        ...
        assert "iterativ/iterativ-deadbeef" in orphaned
        ...
        assert "iterativ/iterativ-deadbeef" not in result.stdout
