...
def _isolate(tmp_path, monkeypatch):
    """Isolate ITERATIV_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal IterativCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = IterativCLI.__new__(IterativCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["iterativ-core"]
        obj.compact = False
