"""Tests for the decomposer module + `iterativ kanban decompose` CLI surface.
...
from iterativ_cli import kanban as kanban_cli
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("iterativ_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("iterativ_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("iterativ_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "iterativ_cli.kanban_decompose._load_config",
