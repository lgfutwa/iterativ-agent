"""Regression tests for TOCTOU-safe credential file writers in ``iterativ_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.iterativ/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks ITERATIV_HOME via
    # get_default_iterativ_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("ITERATIV_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from iterativ_cli import auth as auth_mod
