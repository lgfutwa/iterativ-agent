"""Regression: iterativ doctor must not run a generic Bearer-auth health
...
with HTTP 404. The dedicated check at iterativ_cli/doctor.py already covers
Anthropic with the right headers, so the pluggable profile must be
skipped by `_build_apikey_providers_list()`.

See: IterativKnowledge/iterativ-agent#22346
"""
...
def test_build_apikey_providers_list_skips_dedicated_check_providers():
    from iterativ_cli import doctor
