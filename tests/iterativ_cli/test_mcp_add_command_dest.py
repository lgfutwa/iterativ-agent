"""Regression test: ``iterativ mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``iterativ_cli/_parser.py``).  The dispatcher in ``iterativ_cli/main.py``
...
result, ``iterativ mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the iterativ parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`iterativ mcp add foo --url ...` must keep args.command == "mcp".
