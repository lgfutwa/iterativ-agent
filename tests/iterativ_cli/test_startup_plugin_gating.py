...
``iterativ_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``iterativ --help``, ``iterativ version``,
``iterativ logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``iterativ --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from iterativ_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["iterativ", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["iterativ", "-w", "--tui"], None),
        (["iterativ", "version"], "version"),
        (["iterativ", "--tui", "chat"], "chat"),
        (["iterativ", "-w", "logs"], "logs"),
        (["iterativ", "chat", "hello world"], "chat"),
        (["iterativ", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["iterativ", "-m", "gpt5", "chat"], "chat"),
        (["iterativ", "--model", "gpt5", "chat", "hi"], "chat"),
        (["iterativ", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["iterativ", "-z", "hello world"], None),
        (["iterativ", "-z", "hello", "chat"], "chat"),
        (["iterativ", "--model=gpt5", "chat"], "chat"),     # inline form
        (["iterativ", "--", "chat"], "chat"),               # -- terminator
        (["iterativ", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["iterativ", "some-plugin-cmd"], "some-plugin-cmd"),
        (["iterativ", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["iterativ"],                          # bare → chat
        ["iterativ", "--help"],                # top-level help
        ["iterativ", "-h"],
        ["iterativ", "version"],               # known built-in
        ["iterativ", "logs"],
        ["iterativ", "gateway", "run"],
        ["iterativ", "--tui"],
        ["iterativ", "-w", "--tui"],
        ["iterativ", "chat", "hi"],
        ["iterativ", "help"],                  # accepted built-in-ish
        ["iterativ", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["iterativ", "meet", "join"],          # potential google_meet plugin
        ["iterativ", "honcho", "status"],      # potential memory plugin
        ["iterativ", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``iterativ help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"iterativ_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
