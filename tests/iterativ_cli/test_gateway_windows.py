...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda iterativ_home: "")
    monkeypatch.setattr("iterativ_cli.config.get_iterativ_home", lambda: str(tmp_path / "iterativ-home"))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "iterativ_cli.main"]
