...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ITERATIV_HOME with .container-mode file."""
    iterativ_home = tmp_path / ".iterativ"
    iterativ_home.mkdir()
    monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
    monkeypatch.delenv("ITERATIV_DEV", raising=False)

    container_mode = iterativ_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=iterativ-agent\n"
        "exec_user=iterativ\n"
        "iterativ_bin=/data/current-package/bin/iterativ\n"
    )
    return iterativ_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("iterativ_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "iterativ-agent"
    assert info["exec_user"] == "iterativ"
    assert info["iterativ_bin"] == "/data/current-package/bin/iterativ"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("iterativ_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_iterativ_dev(container_env, monkeypatch):
    """Returns None when ITERATIV_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ITERATIV_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_iterativ_dev_zero(container_env, monkeypatch):
    """ITERATIV_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ITERATIV_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        iterativ_home = Path(tmpdir) / ".iterativ"
        iterativ_home.mkdir()
        (iterativ_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("iterativ_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_iterativ_home": lambda: iterativ_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ITERATIV_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=iterativ-custom\n"
        "exec_user=myuser\n"
        "iterativ_bin=/opt/iterativ/bin/iterativ\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "iterativ-custom"
    assert info["exec_user"] == "myuser"
    assert info["iterativ_bin"] == "/opt/iterativ/bin/iterativ"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from iterativ_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "iterativ"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "iterativ-agent" in cmd
    assert "/data/current-package/bin/iterativ" in cmd
    assert "chat" in cmd
