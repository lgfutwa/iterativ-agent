"""Tests for the Kanban DB layer (iterativ_cli.kanban_db)."""
...
from iterativ_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated ITERATIV_HOME with an empty kanban DB."""
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("ITERATIV_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import iterativ_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from iterativ_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Iterativ profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="iterativ-task", assignee="daily")
...
def test_scratch_workspace_created_under_iterativ_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `iterativ -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's ITERATIV_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's ITERATIV_HOME."""

    def _set_home(self, monkeypatch, tmp_path, iterativ_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
        monkeypatch.delenv("ITERATIV_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_iterativ(
        self, tmp_path, monkeypatch
    ):
        # Standard install: ITERATIV_HOME == ~/.iterativ, no profile active.
        default_home = tmp_path / ".iterativ"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.iterativ/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.iterativ/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.iterativ/kanban.db.
        default_home = tmp_path / ".iterativ"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ITERATIV_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `iterativ -p coder`).
        monkeypatch.setenv("ITERATIV_HOME", str(profile_home))
    ...
    def test_docker_custom_iterativ_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: ITERATIV_HOME points outside ~/.iterativ.
        # `get_default_iterativ_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".iterativ"`.
        custom_root = tmp_path / "opt" / "iterativ"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: ITERATIV_HOME=/opt/iterativ/profiles/coder;
        # `get_default_iterativ_root()` walks up to /opt/iterativ because
    ...
    def test_explicit_override_via_iterativ_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: ITERATIV_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".iterativ"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(profile_home))
        monkeypatch.setenv("ITERATIV_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(default_home))
        monkeypatch.setenv("ITERATIV_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # ITERATIV_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile ITERATIV_HOME and reads.
        monkeypatch.setenv("ITERATIV_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_iterativ_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ITERATIV_KANBAN_DB pins the file path directly and beats both
        # ITERATIV_KANBAN_HOME and the `get_default_iterativ_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(default_home))
        monkeypatch.setenv("ITERATIV_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ITERATIV_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows ITERATIV_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_iterativ_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ITERATIV_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".iterativ"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("ITERATIV_HOME", str(default_home))
        monkeypatch.setenv("ITERATIV_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ITERATIV_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows ITERATIV_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        # ITERATIV_KANBAN_HOME.
        default_home = tmp_path / ".iterativ"
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(default_home))
        monkeypatch.setenv("ITERATIV_KANBAN_DB", "   ")
        monkeypatch.setenv("ITERATIV_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject ITERATIV_KANBAN_DB
        # and ITERATIV_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites ITERATIV_HOME.
        default_home = tmp_path / ".iterativ"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["ITERATIV_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["ITERATIV_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["ITERATIV_KANBAN_TASK"] == "t_dispatch_env"
        assert env["ITERATIV_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see iterativ_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    (see iterativ-agent issue #22032).
    """
    ...
    with _patch("iterativ_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="iterativ_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'iterativ kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_iterativ_argv()
#
# Workers spawned by the dispatcher must use a `iterativ` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["iterativ", ...]`
...
def test_resolve_iterativ_argv_prefers_path_shim(monkeypatch):
    """When `iterativ` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import iterativ_cli.kanban_db as kb

    monkeypatch.delenv("ITERATIV_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/iterativ")
    argv = kb._resolve_iterativ_argv()
    assert argv == ["/usr/local/bin/iterativ"]


def test_resolve_iterativ_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import iterativ_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ITERATIV_BIN", ".\\iterativ.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_iterativ_argv() == [os.path.abspath(".\\iterativ.exe")]


def test_resolve_iterativ_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "iterativ.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("ITERATIV_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_iterativ_argv() == [sys.executable, "-m", "iterativ_cli.main"]


def test_resolve_iterativ_argv_honors_iterativ_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like ITERATIV_BIN lets service managers pin the executable."""
    import shutil
    import iterativ_cli.kanban_db as kb

    shim = tmp_path / "bin" / "iterativ"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("ITERATIV_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_iterativ_argv() == [str(shim)]


def test_resolve_iterativ_argv_iterativ_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare ITERATIV_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import iterativ_cli.kanban_db as kb

    cwd_iterativ = tmp_path / "iterativ"
    cwd_iterativ.write_text("wrong\n", encoding="utf-8")
    cwd_iterativ.chmod(cwd_iterativ.stat().st_mode | stat.S_IXUSR)
    path_iterativ = tmp_path / "bin" / "iterativ"
    path_iterativ.parent.mkdir()
    path_iterativ.write_text("right\n", encoding="utf-8")
    path_iterativ.chmod(path_iterativ.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_iterativ.parent))
    monkeypatch.setenv("ITERATIV_BIN", "iterativ")

    assert kb._resolve_iterativ_argv() == [str(path_iterativ)]


def test_resolve_iterativ_argv_iterativ_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare ITERATIV_BIN does not accept current-directory shadow executables."""
    import sys
    import iterativ_cli.kanban_db as kb

    (tmp_path / "iterativ.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_iterativ_argv_iterativ_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved ITERATIV_BIN batch shim is not used as worker argv[0]."""
    import sys
    import iterativ_cli.kanban_db as kb
...
def test_resolve_iterativ_argv_iterativ_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved ITERATIV_BIN command names do not delegate cwd search to Popen."""
    import sys
    import iterativ_cli.kanban_db as kb
...
def test_resolve_iterativ_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m iterativ_cli.main`.

    Pins the correct module name (NOT `iterativ` — there is no top-level
    `iterativ` package). Regression for #23198: the original PR shipped
    `python -m iterativ` which fails with `No module named iterativ` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_iterativ_argv()
    assert argv == [sys.executable, "-m", "iterativ_cli.main"]


def test_resolve_iterativ_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `iterativ_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m iterativ_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("ITERATIV_BIN", None)
    ...
    assert "Iterativ Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
