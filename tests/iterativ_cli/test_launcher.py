"""Tests for the top-level `./iterativ` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./iterativ` should use `iterativ_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "iterativ"
    called = []

    fake_main_module = types.ModuleType("iterativ_cli.main")

    def fake_main():
        called.append("iterativ_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "iterativ_cli.main", fake_main_module)
    ...
    assert called == ["iterativ_cli.main"]
