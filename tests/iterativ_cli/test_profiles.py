"""Comprehensive tests for iterativ_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ITERATIV_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.iterativ/profiles)
    * ITERATIV_HOME  -> tmp_path/.iterativ  (so get_iterativ_home() agrees)
    * Creates the bare-minimum ~/.iterativ directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["iterativ", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Iterativ install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_iterativ_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".iterativ"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".iterativ" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".iterativ" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.iterativ must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".iterativ"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes iterativ-agent, .worktrees,
        ...
        (default_home / "iterativ-agent" / ".git").mkdir(parents=True)
        (default_home / "iterativ-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "iterativ-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "iterativ-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `iterativ profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `iterativ update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("iterativ_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".iterativ" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_iterativ_home_returns_default(self, profile_env):
        # ITERATIV_HOME points to tmp_path/.iterativ which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".iterativ" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ITERATIV_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ITERATIV_HOME", str(custom))
        # With Docker-aware roots, a custom ITERATIV_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("iterativ")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".iterativ" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("iterativ_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".iterativ" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".iterativ" / "honcho.json"
        ...
        assert "iterativ.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["iterativ.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["iterativ.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "iterativ.ssi_health": {"workspace": "iterativ", "enabled": True}
            }
        }))
        ...
        assert "iterativ.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["iterativ.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["iterativ.heimdall"]["workspace"] == "iterativ"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "iterativ.ssi_health": {"aiPeer": "ssi_health"},
                "iterativ.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["iterativ.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["iterativ.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("iterativ-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".iterativ_history",
        ...
        excluded_prefixes = [
            "default/iterativ-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.iterativ_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultIterativHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_iterativ_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".iterativ" / "profiles"

    def test_default_iterativ_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_iterativ_home()
        assert home == tmp_path / ".iterativ"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ITERATIV_HOME outside ~/.iterativ), profiles go under ITERATIV_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_iterativ_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_iterativ_home() returns ITERATIV_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ITERATIV_HOME under ~/.iterativ), profiles root is still ~/.iterativ/profiles."""
        native = tmp_path / ".iterativ"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ITERATIV_HOME."""
        from iterativ_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ITERATIV_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ITERATIV_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".iterativ" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from iterativ_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".iterativ"
