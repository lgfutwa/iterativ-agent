"""Unit tests for iterativ_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf iterativ-ok"])
        try:
            output = _read_until(bridge, b"iterativ-ok")
            assert b"iterativ-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ITERATIV_PTY_TEST\""],
            env={**os.environ, "ITERATIV_PTY_TEST": "pty-env-works"},
        )
        try:
