...
When ``ITERATIV_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.iterativ/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.iterativ            (has its own auth.json fixture)
    * Profile     -> tmp_path/.iterativ/profiles/coder   (active, ITERATIV_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ITERATIV_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.iterativ as the "real home" — which is NOT equal
    # to the ITERATIV_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    iterativ_home = tmp_path / "classic"
    iterativ_home.mkdir()
    monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

    _write(iterativ_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from iterativ_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ITERATIV_HOME is set to a custom path that is NOT under
    # ~/.iterativ/profiles/ — get_default_iterativ_root() returns ITERATIV_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from iterativ_cli.auth import read_credential_pool, write_credential_pool
