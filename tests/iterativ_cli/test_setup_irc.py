"""Tests for IRC gateway configuration via `iterativ setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Iterativ install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Iterativ install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#iterativ")
            monkeypatch.setenv("IRC_NICKNAME", "iterativ-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `iterativ setup gateway` experience with IRC present."""
