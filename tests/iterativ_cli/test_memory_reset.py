"""Tests for the `iterativ memory reset` CLI command.
...
- Profile-scoped reset (uses ITERATIV_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ITERATIV_HOME with memory files."""
    iterativ_home = tmp_path / ".iterativ"
    memories = iterativ_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nIterativ repo is at ~/.iterativ/iterativ-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `iterativ memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        (iterativ_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ITERATIV_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

        # The memories dir won't exist; get_iterativ_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
