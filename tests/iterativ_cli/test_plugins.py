"""Tests for the Iterativ plugin system (iterativ_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<iterativ_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<iterativ_home>/plugins/``; we derive
    ``<iterativ_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ITERATIV_HOME>/config.yaml.
        # Config is always read from ITERATIV_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        iterativ_home_str = os.environ.get("ITERATIV_HOME")
        if iterativ_home_str:
            iterativ_home = Path(iterativ_home_str)
        else:
            iterativ_home = base.parent
        iterativ_home.mkdir(parents=True, exist_ok=True)
        cfg_path = iterativ_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.iterativ/plugins/ are discovered."""
        plugins_dir = tmp_path / "iterativ_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ITERATIV_HOME", str(tmp_path / "iterativ_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ITERATIV_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".iterativ" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        iterativ_home = tmp_path / "iterativ_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under iterativ_plugins.<name>."""
        plugins_dir = tmp_path / "iterativ_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ITERATIV_HOME", str(tmp_path / "iterativ_test"))

        # Clean up any prior namespace module
        sys.modules.pop("iterativ_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "iterativ_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="iterativ_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "iterativ_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import iterativ_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("iterativ_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / "iterativ_test"
        plugins_dir = iterativ_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "iterativ_test" / "plugins"
        # Set ITERATIV_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("iterativ_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("iterativ_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("iterativ_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("iterativ_cli.plugins.PluginContext.dispatch_tool.__module__", "iterativ_cli.plugins"):
...
class TestPluginDebugLogging:
    """ITERATIV_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ITERATIV_PLUGINS_DEBUG", raising=False)
        from iterativ_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ITERATIV_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ITERATIV_PLUGINS_DEBUG", "1")
        from iterativ_cli import plugins as plugins_mod
