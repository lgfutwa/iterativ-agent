...
The notice prints the most recent curator run summary on `iterativ update`,
...
session start) so users normally never see the rename map. `iterativ update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(iterativ_main)
