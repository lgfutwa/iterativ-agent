...
def server():
    with patch.dict(
        "sys.modules",
        {
            "iterativ_constants": MagicMock(
                get_iterativ_home=MagicMock(return_value="/tmp/iterativ_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'iterativ-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'iterativ-release' patched"
    }
