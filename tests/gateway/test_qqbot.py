...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.iterativ/.update_response."""
        adapter = self._make_adapter()
        iterativ_home = tmp_path / "iterativ_home"
        iterativ_home.mkdir()
        monkeypatch.setattr(
            "iterativ_constants.get_iterativ_home",
            lambda: iterativ_home,
        )
        ...
        response = iterativ_home / ".update_response"
