...
class TestWriteDenyExactPaths:
    ...
    def test_iterativ_env(self):
        # ``.env`` under the active ITERATIV_HOME (profile-aware, not just
        # ``~/.iterativ``) must be write-denied. The hermetic test conftest
        # points ITERATIV_HOME at a tempdir — resolve via get_iterativ_home()
        # to match the denylist.
        from iterativ_constants import get_iterativ_home
        path = str(get_iterativ_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_iterativ_config_not_env(self):
        path = os.path.join(str(Path.home()), ".iterativ", "config.yaml")
