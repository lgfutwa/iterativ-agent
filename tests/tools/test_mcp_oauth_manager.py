...
def test_iterativ_provider_subclass_exists():
    """IterativMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ITERATIV_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ITERATIV_PROVIDER_CLS is not None
    assert issubclass(_ITERATIV_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_iterativ_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns IterativMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ITERATIV_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ITERATIV_HOME", str(tmp_path))
    ...
    assert _ITERATIV_PROVIDER_CLS is not None
    assert isinstance(provider, _ITERATIV_PROVIDER_CLS)
    assert provider._iterativ_server_name == "srv"
