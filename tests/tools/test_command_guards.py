...
def _clean_state():
    ...
    saved = {}
    for k in ("ITERATIV_INTERACTIVE", "ITERATIV_GATEWAY_SESSION", "ITERATIV_EXEC_ASK", "ITERATIV_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ITERATIV_INTERACTIVE"] = "1"
        session_key = os.getenv("ITERATIV_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ITERATIV_INTERACTIVE or ITERATIV_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
