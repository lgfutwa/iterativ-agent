...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_iterativ_home_override(self, tmp_path):
        """Auth lookup should read from ITERATIV_HOME/auth.json, not ~/.iterativ/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".iterativ").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "ITERATIV_HOME": str(iterativ_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``iterativ tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
