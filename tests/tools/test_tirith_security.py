...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._iterativ_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_iterativ_bin(self):
        """After _INSTALL_FAILED, manual install in ITERATIV_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        iterativ_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(iterativ_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(iterativ_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._iterativ_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == iterativ_bin
            assert _tirith_mod._resolved_path == iterativ_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ITERATIV_HOME isolation
# ---------------------------------------------------------------------------

class TestIterativHomeIsolation:
    def test_iterativ_bin_dir_respects_iterativ_home(self):
        """_iterativ_bin_dir must use ITERATIV_HOME, not hardcoded ~/.iterativ."""
        from tools.tirith_security import _iterativ_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ITERATIV_HOME": tmpdir}):
            result = _iterativ_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_iterativ_home(self):
        """_failure_marker_path must use ITERATIV_HOME, not hardcoded ~/.iterativ."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ITERATIV_HOME": "/custom/iterativ"}):
            result = _failure_marker_path()
        assert result == "/custom/iterativ/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ITERATIV_HOME; verify it's active."""
        iterativ_home = os.getenv("ITERATIV_HOME")
        assert iterativ_home is not None, "ITERATIV_HOME should be set by conftest"
        assert "iterativ_test" in iterativ_home, "Should point to test temp dir"

    def test_get_iterativ_home_fallback(self):
        """Without ITERATIV_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_iterativ_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ITERATIV_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".iterativ")
            result = _get_iterativ_home()
