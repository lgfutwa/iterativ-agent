...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        iterativ_home.mkdir()
        (iterativ_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(iterativ_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.iterativ/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        (iterativ_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        (iterativ_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        (iterativ_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        iterativ_home = tmp_path / ".iterativ"
        skills_dir = iterativ_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.iterativ/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        iterativ_home = tmp_path / ".iterativ"
        (iterativ_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ITERATIV_HOME": str(iterativ_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.iterativ")

        assert mounts[0]["container_path"] == "/home/user/.iterativ/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.iterativ/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.iterativ/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ITERATIV_HOME."""
        monkeypatch.setenv("ITERATIV_HOME", str(tmp_path / ".iterativ"))
        (tmp_path / ".iterativ").mkdir()

        # Create a sensitive file one level above iterativ_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside iterativ_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ITERATIV_HOME must still be registered."""
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
        (iterativ_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_iterativ_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ITERATIV_HOME must be allowed."""
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        subdir = iterativ_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ITERATIV_HOME pointing outside must be rejected."""
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

        # Create a sensitive file outside iterativ_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside iterativ_home pointing outside
        symlink = iterativ_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ITERATIV_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, iterativ_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ITERATIV_HOME."""
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))
        ...
        sensitive.write_text("{}")
        self._write_config(iterativ_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(iterativ_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ITERATIV_HOME via config must still mount."""
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

        (iterativ_home / "oauth.json").write_text("{}")
        self._write_config(iterativ_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        (iterativ_home / "cache" / "documents").mkdir(parents=True)
        (iterativ_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.iterativ/cache/documents" in paths
        assert "/root/.iterativ/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.iterativ/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        iterativ_home.mkdir()
        # Use legacy dir name — get_iterativ_dir prefers old if it exists
        (iterativ_home / "document_cache").mkdir()
        (iterativ_home / "image_cache").mkdir()
        monkeypatch.setenv("ITERATIV_HOME", str(iterativ_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(iterativ_home / "document_cache") in host_paths
        assert str(iterativ_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.iterativ/cache/documents" in container_paths
        assert "/root/.iterativ/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        doc_dir = iterativ_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        iterativ_home = tmp_path / ".iterativ"
        ss_dir = iterativ_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.iterativ/cache/screenshots/session_abc/screen1.png"
