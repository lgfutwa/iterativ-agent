...
def _restore_tool_modules():
    original_iterativ_home = os.environ.get("ITERATIV_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "iterativ_cli"
        or name.startswith("iterativ_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_iterativ_home is None:
            os.environ.pop("ITERATIV_HOME", None)
        else:
            os.environ["ITERATIV_HOME"] = original_iterativ_home
        _reset_modules(("tools", "iterativ_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    iterativ_cli = types.ModuleType("iterativ_cli")
    iterativ_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["iterativ_cli"] = iterativ_cli
    iterativ_home = tmp_path / "iterativ-home"
    os.environ["ITERATIV_HOME"] = str(iterativ_home)
    sys.modules["iterativ_cli.config"] = types.SimpleNamespace(
        get_iterativ_home=lambda: iterativ_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="iterativ-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": iterativ_home / "modal_snapshots.json",
