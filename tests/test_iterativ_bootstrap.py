"""Tests for iterativ_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Iterativ entry point
(iterativ, iterativ-agent, iterativ-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Iterativ entry point imports iterativ_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported iterativ_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("iterativ_bootstrap", None)
    import iterativ_bootstrap  # noqa: WPS433
    return iterativ_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Iterativ CLI
...
class TestEntryPointsImportBootstrap:
    """Every Iterativ entry point must import iterativ_bootstrap as its
    ...
    # Entry points that invoke Iterativ as a process.  Each one must
    # import iterativ_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "iterativ_cli/main.py",   # iterativ CLI (console_script)
        "run_agent.py",          # iterativ-agent (console_script)
        "acp_adapter/entry.py",  # iterativ-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import iterativ_bootstrap' and that
        ...
        half-finished ``iterativ update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``iterativ_bootstrap`` as a top-level module) leaves iterativ
        ...
        """
        # Resolve relative to the iterativ-agent repo root.  Tests live
        # at tests/test_iterativ_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # iterativ start even when iterativ_bootstrap hasn't been
        ...
        assert first_import_name == "iterativ_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'iterativ_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import iterativ_bootstrap' line to be the first import."
        )
