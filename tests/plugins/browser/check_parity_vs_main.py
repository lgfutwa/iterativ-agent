...
Run from the PR worktree:
    ...
    cd ~/.iterativ/iterativ-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.iterativ/iterativ-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.iterativ/iterativ-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a iterativ-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a iterativ-agent checkout"
)
...
# Isolated ITERATIV_HOME for the config write.
