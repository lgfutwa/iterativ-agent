...
# Bootstrap Open WebUI against Iterativ Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.iterativ/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-iterativ.sh
...
#   OPEN_WEBUI_NAME='Johnny Iterativ'
...
#   ITERATIV_API_PORT=8642
#   ITERATIV_API_HOST=127.0.0.1
#   ITERATIV_API_MODEL_NAME='Iterativ Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Iterativ Agent WebUI}"
...
ITERATIV_ENV_FILE="${ITERATIV_ENV_FILE:-$HOME/.iterativ/.env}"
ITERATIV_API_PORT="${ITERATIV_API_PORT:-8642}"
ITERATIV_API_HOST="${ITERATIV_API_HOST:-127.0.0.1}"
ITERATIV_API_CONNECT_HOST="${ITERATIV_API_CONNECT_HOST:-127.0.0.1}"
ITERATIV_API_MODEL_NAME="${ITERATIV_API_MODEL_NAME:-Iterativ Agent}"
ITERATIV_API_BASE_URL="http://${ITERATIV_API_CONNECT_HOST}:${ITERATIV_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-iterativ.sh"
LOG_DIR="$HOME/.iterativ/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ITERATIV_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ITERATIV_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.iterativ.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.iterativ</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.iterativ"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.iterativ"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-iterativ.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Iterativ Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-iterativ.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.iterativ/logs/openwebui.log
StandardError=append:%h/.iterativ/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-iterativ.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ITERATIV_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Iterativ API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ITERATIV_ENV_FILE"
  upsert_env API_SERVER_HOST "$ITERATIV_API_HOST" "$ITERATIV_ENV_FILE"
  upsert_env API_SERVER_PORT "$ITERATIV_API_PORT" "$ITERATIV_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ITERATIV_API_MODEL_NAME" "$ITERATIV_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ITERATIV_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Iterativ gateway so API server settings take effect...'
  iterativ gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ITERATIV_API_CONNECT_HOST}:${ITERATIV_API_PORT}/health" >/dev/null; then
    log 'Iterativ API server did not answer on the first check. Trying to start gateway in the background...'
    nohup iterativ gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ITERATIV_API_CONNECT_HOST}:${ITERATIV_API_PORT}/health" >/dev/null
  ...
  log "Iterativ API endpoint: ${ITERATIV_API_BASE_URL}"
