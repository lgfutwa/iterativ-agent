#!/usr/bin/env bash
# Canonical test runner for iterativ-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.iterativ/iterativ-agent/venv"; do
...
# Unset ITERATIV_* behavioral vars too.
unset ITERATIV_YOLO_MODE ITERATIV_INTERACTIVE ITERATIV_QUIET ITERATIV_TOOL_PROGRESS \
      ITERATIV_TOOL_PROGRESS_MODE ITERATIV_MAX_ITERATIONS ITERATIV_SESSION_PLATFORM \
      ITERATIV_SESSION_CHAT_ID ITERATIV_SESSION_CHAT_NAME ITERATIV_SESSION_THREAD_ID \
      ITERATIV_SESSION_SOURCE ITERATIV_SESSION_KEY ITERATIV_GATEWAY_SESSION \
      ITERATIV_CRON_SESSION \
      ITERATIV_PLATFORM ITERATIV_INFERENCE_PROVIDER ITERATIV_MANAGED ITERATIV_DEV \
      ITERATIV_CONTAINER ITERATIV_EPHEMERAL_SYSTEM_PROMPT ITERATIV_TIMEZONE \
      ITERATIV_REDACT_SECRETS ITERATIV_BACKGROUND_NOTIFICATIONS ITERATIV_EXEC_ASK \
      ITERATIV_HOME_MODE 2>/dev/null || true
...
# If a system-wide iterativ pytest_live_guard plugin is installed at
# $HOME/.iterativ/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.iterativ/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.iterativ:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.iterativ" ;;
  esac
...
WORKERS="${ITERATIV_TEST_WORKERS:-4}"
