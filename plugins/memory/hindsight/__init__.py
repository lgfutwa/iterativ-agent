...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: iterativ)
...
def _load_config() -> dict:
    ...
    profile_path = get_iterativ_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "iterativ": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "iterativ"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "iterativ")
    return str(profile or "iterativ")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``iterativ-{user}`` with no user becomes ``iterativ``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, iterativ_home):
        """Write config to $HERMES_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(iterativ_home) / "hindsight"
    ...
    def post_setup(self, iterativ_home: str, config: dict) -> None:
        ...
        from iterativ_cli.memory_setup import _curses_select
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(iterativ_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "iterativ")
        ...
        self.save_config(provider_config, iterativ_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(iterativ_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(iterativ_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "iterativ"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: iterativ-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "iterativ"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "iterativ"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "iterativ", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "iterativ")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See iterativ-agent#6672.
