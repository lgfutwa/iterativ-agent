...
_DEFAULT_CONTAINER_TAG = "iterativ"
...
def _load_supermemory_config(iterativ_home: str) -> dict:
    config = _default_config()
    config_path = Path(iterativ_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, iterativ_home: str) -> None:
    config_path = Path(iterativ_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `iterativ memory setup`.
    ...
    def save_config(self, values, iterativ_home):
        ...
        _save_supermemory_config(sanitized, iterativ_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from iterativ_constants import get_iterativ_home
        self._iterativ_home = kwargs.get("iterativ_home") or str(get_iterativ_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._iterativ_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "iterativ", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "iterativ_memory", "target": target, "type": "explicit_memory"},
