...
Handles: iterativ honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "iterativ" else ""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'iterativ honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'iterativ honcho setup' first.\n")
        return
    ...
    for p in profiles:
        ...
        if clone_honcho_for_profile(p.name):
            print(f"  + {p.name} -> iterativ.{p.name}")
            created += 1
        else:
            skipped += 1
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `iterativ update` -- no output, no exceptions.
    """
    try:
        from iterativ_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    """
    return get_iterativ_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    iterativ_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'iterativ honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = iterativ_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        iterativ_host["peerName"] = new_peer

    current_ai = iterativ_host.get("aiPeer") or cfg.get("aiPeer", "iterativ")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        iterativ_host["aiPeer"] = new_ai

    current_workspace = iterativ_host.get("workspace") or cfg.get("workspace", "iterativ")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        iterativ_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = iterativ_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        iterativ_host["observationMode"] = new_obs
    else:
        iterativ_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(iterativ_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        iterativ_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        iterativ_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = iterativ_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        iterativ_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = iterativ_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        iterativ_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                iterativ_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(iterativ_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            iterativ_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        iterativ_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        iterativ_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        iterativ_host["dialecticReasoningLevel"] = new_reasoning
    else:
        iterativ_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = iterativ_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        iterativ_host["sessionStrategy"] = new_strat

    iterativ_host["enabled"] = True
    iterativ_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        iterativ_config = load_config()
        iterativ_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(iterativ_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: iterativ config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    iterativ honcho status     -- show full config")
    print("    iterativ honcho mode       -- change recall/observation mode")
    print("    iterativ honcho tokens     -- tune context and dialectic budgets")
    print("    iterativ honcho peer       -- update peer names")
    print("    iterativ honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: iterativ honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'iterativ honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: iterativ honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        iterativ = hosts.get(_host_key(), {})
        user = iterativ.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = iterativ.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = iterativ.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = iterativ.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Iterativ' identity in Honcho. Seed with 'iterativ honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: iterativ honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: iterativ honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    cfg = _read_config()
    hosts = cfg.get("hosts", {})
    iterativ_hosts = hosts.get(_host_key(), {})

    context = getattr(args, "context", None)
    dialectic = getattr(args, "dialectic", None)

    if context is None and dialectic is None:
        ctx_tokens = iterativ.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = iterativ.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = iterativ.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("\n  Set with: iterativ honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'iterativ honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'iterativ honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    iterativ honcho identity --show        — show both peer representations")
        print("    iterativ honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  iterativ honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'iterativ honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'iterativ honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  iterativ honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'iterativ'")
        ...
        for f in user_files:
            print("    iterativ honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'iterativ honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'iterativ honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    iterativ honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  iterativ honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  iterativ honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. iterativ honcho setup              — configure API key (required)")
        print("  2. iterativ honcho migrate            — re-run this walkthrough")
    else:
        print("  1. iterativ honcho status             — verify Honcho connection")
        print("  2. iterativ                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. iterativ honcho identity --show    — verify AI peer representation")
        print("  4. iterativ honcho tokens             — tune context and dialectic budgets")
        print("  5. iterativ honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'iterativ memory setup'...\n")
        from iterativ_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``iterativ honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``iterativ honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to iterativ memory setup)",
    )
