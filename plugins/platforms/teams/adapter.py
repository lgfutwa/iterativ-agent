...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        iterativ_action = data.get("iterativ_action", "")
        session_key = data.get("session_key", "")

        if not iterativ_action or not session_key:
        ...
        choice = choice_map.get(iterativ_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="iterativ_approve",
                    data={**btn_data_base, "iterativ_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="iterativ_approve",
                    data={**btn_data_base, "iterativ_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="iterativ_approve",
                    data={**btn_data_base, "iterativ_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="iterativ_approve",
                    data={**btn_data_base, "iterativ_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.iterativ/.env")
