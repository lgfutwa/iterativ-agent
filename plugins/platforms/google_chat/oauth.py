...
try:
    from iterativ_constants import display_iterativ_home, get_iterativ_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where iterativ_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _iterativ_home.py shim).
...
# ``ls ~/.iterativ/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _iterativ_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _iterativ_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _iterativ_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _iterativ_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _iterativ_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install iterativ-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'iterativ-agent[google_chat]'")
        return False
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_iterativ_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_iterativ_home()}/google_chat_user_token.json"
    )
