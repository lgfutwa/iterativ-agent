...
discoverable and `iterativ setup` can describe it even when websockets is
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"iterativ-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``iterativ setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.iterativ/.env`` via ``iterativ_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("iterativ_cli.config not available; set SIMPLEX_* vars manually in ~/.iterativ/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `iterativ gateway status` without
