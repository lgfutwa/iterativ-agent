...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "iterativ-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: iterativ_, iterativ_1, iterativ_2...
...
def check_requirements() -> bool:
    ...
    # iterativ setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `iterativ gateway setup` flow for the IRC platform.

    Lazy-imports ``iterativ_cli.setup`` helpers so the plugin stays importable
    ...
    channel = prompt(
        "Channel to join (e.g. #iterativ — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.iterativ/.env")
    print_info("Restart the gateway for changes to take effect: iterativ gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "iterativ-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "iterativ-bot"
