...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            iterativ_home = Path.home().joinpath(".iterativ").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``iterativ status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``iterativ status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``iterativ setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.iterativ/.env`` via ``iterativ_cli.config``.
    """
    print()
    ...
    try:
        from iterativ_cli.config import get_env_var, set_env_var
    except ImportError:
        print("iterativ_cli.config not available; set LINE_* vars manually in ~/.iterativ/.env")
        return
