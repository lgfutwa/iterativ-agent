"""`iterativ meet node ...` subcommand tree.

Wired into the existing ``iterativ meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``iterativ meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="iterativ-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``iterativ meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             iterativ meet node approve <name> ws://<host>:{args.port} {token}")
        try:
