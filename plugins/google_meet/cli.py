...
Wires ``iterativ meet <subcommand>``:
...
def _auth_state_path() -> Path:
    return Path(get_iterativ_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``iterativ meet`` argparse tree.
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"iterativ meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: iterativ meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: iterativ meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: iterativ meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "iterativ meet join https://meet.google.com/abc-defg-hij"
        )
    else:
    ...
    if realtime:
        ...
        if system == "Linux":
            ...
            print(
                ...
                "  iterativ will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: iterativ meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: iterativ meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="iterativ meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1
