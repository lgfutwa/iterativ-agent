...
``$ITERATIV_HOME/workspace/meetings/<meeting-id>/``. The main iterativ process
...
# and exposes ``window.__iterativMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__iterativMeetInstalled) return;
  window.__iterativMeetInstalled = true;
  window.__iterativMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__iterativMeetQueue.push({
      ts: Date.now(),
  ...
  window.__iterativMeetDrain = () => {
    const out = window.__iterativMeetQueue.slice();
    window.__iterativMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "iterativ_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
    ...
    try:
        with sync_playwright() as pw:
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__iterativMeetDrain && window.__iterativMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__iterativMeetInstalled) {
