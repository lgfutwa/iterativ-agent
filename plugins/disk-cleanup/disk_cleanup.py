...
Rules:
  ...
  - empty dirs    → always delete (under ITERATIV_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ITERATIV_HOME and /tmp/iterativ-*
Never touches: ~/.iterativ/logs/ or any system directory.
"""
...
try:
    ...
    def get_iterativ_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ITERATIV_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".iterativ").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ITERATIV_HOME/logs/``."""
    return get_iterativ_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ITERATIV_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ITERATIV_HOME or ``/tmp/iterativ-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    iterativ_home = get_iterativ_home()
    try:
        path.resolve().relative_to(iterativ_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/iterativ-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("iterativ-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ITERATIV_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ITERATIV_HOME (but leave ITERATIV_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "iterativ-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(iterativ_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == iterativ_home:
                continue
            try:
                rel_parts = dirpath.relative_to(iterativ_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    iterativ_home = get_iterativ_home()
    try:
        rel = path.resolve().relative_to(iterativ_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "iterativ-agent",
        }:
            return None
        ...
        # Path isn't under ITERATIV_HOME (e.g. /tmp/iterativ-*) — fall through.
        pass
