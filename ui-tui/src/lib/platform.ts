 ...
 * iterativ-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``iterativ_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but iterativ-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS iterativ-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on iterativ-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is iterativ-ink's Alt signal and accepting it would
