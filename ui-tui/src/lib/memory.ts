...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.ITERATIV_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.iterativ', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `iterativ-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
