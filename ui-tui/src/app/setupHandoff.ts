import type { RunExternalProcess } from '@iterativ/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`iterativ ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching iterativ: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`iterativ ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
