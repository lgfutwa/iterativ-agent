"""Centralized logging setup for Iterativ Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.iterativ/logs/`` (profile-aware via ``get_iterativ_home()``).
...
from iterativ_constants import get_config_path, get_iterativ_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_iterativ_session_injector", False):
    ...
    _session_record_factory._iterativ_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``iterativ logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "iterativ_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("iterativ_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the Iterativ logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    iterativ_home
        Override for the Iterativ home directory.  Falls back to
        ``get_iterativ_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_iterativ_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._iterativ_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the iterativ group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from iterativ_cli.config import is_managed
