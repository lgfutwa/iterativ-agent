...
description: "Reference for Iterativ Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
- [Use MCP with Iterativ](/docs/guides/use-mcp-with-iterativ)
...
Iterativ Agent may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Iterativ Agent only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Iterativ Agent does not create an empty MCP runtime toolset for that server.
