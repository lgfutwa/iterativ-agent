"""ACP auth helpers — detect and advertise Iterativ authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "iterativ-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Iterativ runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Iterativ can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Iterativ.
    ...
    not have Iterativ provider credentials configured yet, so Iterativ always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Iterativ using the currently configured "
