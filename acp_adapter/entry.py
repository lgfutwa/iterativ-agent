"""CLI entry point for the iterativ-agent ACP adapter.

Loads environment variables from ``~/.iterativ/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ITERATIV_HOME (default ``~/.iterativ``)."""
    from iterativ_cli.env_loader import load_iterativ_dotenv

    iterativ_home = get_iterativ_home()
    loaded = load_iterativ_dotenv(iterativ_home=iterativ_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", iterativ_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="iterativ-acp",
        description="Run Iterativ Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Iterativ version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(iterativ_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import IterativACPAgent  # noqa: F401

    print("Iterativ ACP check OK")


def _run_setup() -> None:
    from iterativ_cli.main import main as iterativ_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "iterativ", "model"]
        iterativ_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``iterativ postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting iterativ-agent ACP adapter")
    ...
    import acp
    from .server import IterativACPAgent
