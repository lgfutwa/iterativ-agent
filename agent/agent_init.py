...
from iterativ_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.iterativ/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from iterativ_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.iterativ/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from iterativ_logging import setup_logging, setup_verbose_logging
    setup_logging(iterativ_home=_ra()._iterativ_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that iterativ_logging.setup_logging() does not
        ...
        # handler level inside iterativ_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from iterativ_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `iterativ model` to "
                        "select a provider, or run `iterativ setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ITERATIV_KANBAN_TASK is set).
    ...
    os.environ["ITERATIV_SESSION_ID"] = agent.session_id
    try:
    ...
    # Session logs go into ~/.iterativ/sessions/ alongside gateway sessions
    iterativ_home = get_iterativ_home()
    agent.logs_dir = iterativ_home / "sessions"
    ...
    try:
        from iterativ_cli.config import load_config as _load_agent_config
    ...
    # Knowledge II graph substrate. This is additive and fail-open: if graph
    # setup fails, the classic memory/tool flow continues unchanged.
    agent._knowledge_graph = None
    _knowledge_cfg = {}
    try:
        _knowledge_cfg = _agent_cfg.get("knowledge", {}) or {}
        if _knowledge_cfg.get("enabled", True):
            if (
                session_db is not None
                and not _knowledge_cfg.get("db_path")
                and hasattr(session_db, "get_knowledge_graph")
            ):
                agent._knowledge_graph = session_db.get_knowledge_graph()
            else:
                from knowledge.hooks import get_knowledge_graph_from_config

                agent._knowledge_graph = get_knowledge_graph_from_config(_knowledge_cfg)
    except Exception:
        agent._knowledge_graph = None
        ...
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                ...
                agent._memory_store = MemoryStore(
                    ...
                    knowledge_graph=(
                        agent._knowledge_graph
                        if _knowledge_cfg.get("capture_memory_writes", True)
                        else None
                    ),
                )
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "iterativ"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from iterativ_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from iterativ_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Iterativ Agent.  Choose a model with at least "
