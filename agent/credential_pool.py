...
from iterativ_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``iterativ model`` / ``iterativ auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Iterativ process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``iterativ`` invocation that defaults to the active provider
        (e.g. setup wizard, ``iterativ auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("iterativ_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Iterativ
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("iterativ_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Iterativ process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Iterativ profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `iterativ model` / `iterativ auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `iterativ model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `iterativ auth remove <provider> <N>` is stable across all source types.
    try:
        from iterativ_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Iterativ PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_iterativ_oauth_credentials

        for source_name, creds in (
            ("iterativ_pkce", read_iterativ_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `iterativ auth add nous --label <name>`).  Fall back to the
        ...
        # Iterativ auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from iterativ_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.iterativ/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `iterativ auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `iterativ auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Iterativ auth store.  Without this gate the removal is instantly
        ...
        # Iterativ owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `iterativ auth openai-codex`.
        ...
        # When the user logs in via ``iterativ model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``iterativ auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.iterativ/.env over os.environ — the user's config file is the
    # authoritative source for Iterativ credentials. Stale env vars from parent
    ...
    # Honour user suppression — `iterativ auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.iterativ/.env.
    ...
    try:
        from iterativ_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.iterativ/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "iterativ_pkce"}
        )
    ]
    if len(retained) == len(entries):
        return False
