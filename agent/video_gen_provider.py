...
as ``kind: backend``) or ``~/.iterativ/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``iterativ tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``iterativ tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``iterativ tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``iterativ tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ITERATIV_HOME/cache/videos/``, creating parents as needed."""
    from iterativ_constants import get_iterativ_home

    path = get_iterativ_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ITERATIV_HOME/cache/videos/``.
