...
from iterativ_constants import get_iterativ_home, get_skills_dir, is_wsl
...
_ITERATIV_MD_NAMES = (".iterativ.md", "ITERATIV.md")


def _find_iterativ_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.iterativ.md`` or ``ITERATIV.md``.
    ...
    for directory in [current, *current.parents]:
        for name in _ITERATIV_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Iterativ Agent, an intelligent AI assistant created by Iterativ Knowledge. "
...
ITERATIV_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Iterativ Agent "
    "itself, load the `iterativ-agent` skill with skill_view(name='iterativ-agent') "
    "before answering. Docs: https://iterativ-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.iterativ/kanban.db`. Your task id is in "
    "`$ITERATIV_KANBAN_TASK`; your workspace is `$ITERATIV_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ITERATIV_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `iterativ kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Iterativ WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Iterativ itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Iterativ runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Iterativ itself is running. The host OS, home, and cwd "
                f"of the Iterativ process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Iterativ "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_iterativ_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.iterativ/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ITERATIV_PLATFORM")
        or get_session_env("ITERATIV_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Iterativ Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `iterativ-agent` skill "
            "first. It has the actual commands (e.g. `iterativ config set …`, `iterativ tools`, "
            "`iterativ setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: iterativ setup, iterativ setup tools, iterativ setup terminal, iterativ status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ITERATIV_HOME and return its content, or None.
    ...
    """
    try:
        from iterativ_cli.config import ensure_iterativ_home
        ensure_iterativ_home()
    except Exception as e:
        logger.debug("Could not ensure ITERATIV_HOME before loading SOUL.md: %s", e)

    soul_path = get_iterativ_home() / "SOUL.md"
...
def _load_iterativ_md(cwd_path: Path) -> str:
    """.iterativ.md / ITERATIV.md — walk to git root."""
    iterativ_md_path = _find_iterativ_md(cwd_path)
    if not iterativ_md_path:
        return ""
    try:
        content = iterativ_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = iterativ_md_path.name
        try:
            rel = str(iterativ_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".iterativ.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", iterativ_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .iterativ.md / ITERATIV.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ITERATIV_HOME is independent and always included when present.
    ...
    # SOUL.md from ITERATIV_HOME only — skip when already loaded as identity
    if not skip_soul:
