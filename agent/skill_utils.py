...
from iterativ_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ITERATIV_PLATFORM`` or
            ``ITERATIV_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("ITERATIV_PLATFORM")
...
# which becomes the dominant cost of ``iterativ`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.iterativ/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``iterativ`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ITERATIV_HOME, not cwd
        if not p.is_absolute():
            p = (iterativ_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.iterativ/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    iterativ = metadata.get("iterativ") or {}
    if not isinstance(iterativ, dict):
        iterativ = {}
    return {
        "fallback_for_toolsets": iterativ.get("fallback_for_toolsets", []),
        "requires_toolsets": iterativ.get("requires_toolsets", []),
        "fallback_for_tools": iterativ.get("fallback_for_tools", []),
        "requires_tools": iterativ.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    iterativ = metadata.get("iterativ")
    if not isinstance(iterativ, dict):
        return []
    raw = iterativ.get("config")
    if not raw:
        return []
