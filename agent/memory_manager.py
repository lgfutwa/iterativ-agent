...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``iterativ_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_iterativ_home()`` themselves.
        """
        if "iterativ_home" not in kwargs:
            from iterativ_constants import get_iterativ_home
            kwargs["iterativ_home"] = str(get_iterativ_home())
        for provider in self._providers:
            try:
