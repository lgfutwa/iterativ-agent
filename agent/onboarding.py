...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Iterativ starts and finds ``~/.openclaw/``.

    Points users at ``iterativ claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``iterativ claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Iterativ, run "
        "`iterativ claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `iterativ claw cleanup` (renames it to ~/.openclaw.pre-migration — "
