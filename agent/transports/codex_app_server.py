...
projection into Iterativ' display, approval bridging, transcript projection into
...
"codex_app_server"`. Iterativ' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ITERATIV_KANBAN_TASK"):
            kanban_db = spawn_env.get("ITERATIV_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "ITERATIV_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ITERATIV_HOME", os.path.expanduser("~/.iterativ")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "iterativ",
        client_title: str = "Iterativ Agent",
