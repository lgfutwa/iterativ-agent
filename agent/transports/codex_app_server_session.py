...
Owns one Codex thread per Iterativ session. Drives `turn/start`, consumes
...
# Iterativ' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ITERATIV_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Iterativ session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ITERATIV_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ITERATIV_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="iterativ",
            client_title="Iterativ Agent",
            client_version=_get_iterativ_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Iterativ' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Iterativ' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own iterativ-tools callback we
            # auto-accept — the user already approved Iterativ' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Iterativ approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Iterativ returns 'once', 'session', 'always', or 'deny'.
...
def _get_iterativ_version() -> str:
    """Best-effort Iterativ version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("iterativ-agent")
