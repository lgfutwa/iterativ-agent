...
as ``kind: backend``) or ``~/.iterativ/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$ITERATIV_HOME/cache/images/``, creating parents as needed."""
    from iterativ_constants import get_iterativ_home

    path = get_iterativ_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ITERATIV_HOME/cache/images/``.
