...
Storage
-------
Bundles live in ``~/.iterativ/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ITERATIV_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``iterativ bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ITERATIV_HOME.

    Honors ``ITERATIV_BUNDLES_DIR`` for tests; falls back to
    ``<ITERATIV_HOME>/skill-bundles``.
    """
    override = os.environ.get("ITERATIV_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_iterativ_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `iterativ bundles` CLI subcommand.
# ---------------------------------------------------------------------------
