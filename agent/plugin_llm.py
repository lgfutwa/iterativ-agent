...
Plugins built on Iterativ Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Iterativ
...
:class:`~iterativ_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Iterativ' main config
...
handles every provider, fallback chain, and per-task override Iterativ
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`iterativ_cli.plugins.PluginContext`
