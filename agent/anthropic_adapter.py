"""Anthropic Messages API adapter for Iterativ Agent.

Translates between Iterativ's internal OpenAI-style message format and
...
# Iterativ effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See iterativ-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Iterativ historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Iterativ)
    ...
    # 1. Iterativ-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Iterativ configs.
...
# ── Iterativ-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.iterativ/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ITERATIV_OAUTH_FILE = get_iterativ_home() / ".anthropic_oauth.json"
...
def run_iterativ_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Iterativ-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Iterativ with your Claude Pro/Max subscription.")
    print()
...
def read_iterativ_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Iterativ-managed OAuth credentials from ~/.iterativ/.anthropic_oauth.json."""
    if _ITERATIV_OAUTH_FILE.exists():
        try:
            data = json.loads(_ITERATIV_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Iterativ OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See iterativ-agent#13848.
            #
    ...
    # are preserved.  See iterativ-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Iterativ Agent", "Claude Code")
                text = text.replace("Iterativ agent", "Claude Code")
                text = text.replace("iterativ-agent", "claude-code")
                text = text.replace("Iterativ Knowledge", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Iterativ surfaces in its CLI. We
