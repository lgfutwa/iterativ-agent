    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if iterativ_home_str:
            takeover_path = Path(iterativ_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(iterativ_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded iterativ-agent but never re-ran ``iterativ setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../iterativ-gateway.service"
    ...
    # that's the common case for iterativ.
    ...
    # constant in iterativ_cli/gateway.py.
    headroom = 30.0
