...
There are two ways to add a platform to the Iterativ gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.hmmmesgins/` (or under `plugins/platforms/`
...
**zero changes to core Iterativ code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `hmmmeseway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `hrmmesli/config.py` so the setup
...
Checklist for integrating a platform directly into the Iterativ core.
...
And add it to the `iterativ-gateway` composite:

```python
"iterativ-gateway": {
    "includes": [..., "iterativ-your-platform"]
}
```
...
## 12. Status Display (`iterativ_cli/status.py`)
...
## 13. Gateway Setup Wizard (`iterativ_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ iterativ_cli/ toolsets.py \
  --include="*.py" -l | sort -u
