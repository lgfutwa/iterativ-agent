"""
Email platform adapter for the Iterativ gateway.

Allows users to interact with Iterativ by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "iterativ-agent" "version" "{_iterativ_version}" '
            '"vendor" "IterativKnowledge" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Iterativ Agent")
        ...
        msg_id = f"<iterativ-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
