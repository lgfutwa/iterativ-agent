# Contributing to Iterativ Agent

Thank you for contributing to Iterativ Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Iterativ to work everywhere.
...
Bundled skills (in `skills/`) ship with every Iterativ install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `iterativ skills browse` (labeled "official") and install it with `iterativ skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Iterativ Knowledge Discord](https://discord.gg/IterativKnowledge). Users can install it with `iterativ skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.iterativ/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(iterativ_home, config)` for setup-wizard integration
...
- Integrate with `iterativ memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/IterativKnowledge/iterativ-agent.git
cd iterativ-agent
...
```bash
mkdir -p ~/.iterativ/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.iterativ/config.yaml
touch ~/.iterativ/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.iterativ/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/iterativ" ~/.local/bin/iterativ

# Verify
iterativ doctor
iterativ chat -q "Hello"
```
...
├── cli.py                    # IterativCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (iterativ-cli, iterativ-telegram, etc.)
├── iterativ_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── iterativ_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.iterativ/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (iterativ-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.iterativ/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.iterativ/`)

| Path | Purpose |
|------|---------|
| `~/.iterativ/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.iterativ/.env` | API keys and secrets |
| `~/.iterativ/auth.json` | OAuth credentials (Nous Portal) |
| `~/.iterativ/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.iterativ/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.iterativ/state.db` | SQLite session database |
| `~/.iterativ/sessions/` | JSON session logs |
| `~/.iterativ/cron/` | Scheduled job data |
| `~/.iterativ/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`iterativ_state.py`) with full-text search and unique session titles. JSON logs go to `~/.iterativ/sessions/`.
...
(for example `_ITERATIV_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.iterativ`:
...
The user may skip setup and keep loading the skill. Iterativ only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `iterativ setup` or update `~/.iterativ/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Iterativ tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Iterativ Agent" is the secondary collaborator. If the contributor's commit shows "Iterativ Agent" as author because they used Iterativ to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Iterativ tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `iterativ --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Iterativ uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.iterativ/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `iterativ_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `iterativ_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Iterativ runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the iterativ wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `iterativ_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `iterativ_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `iterativ_cli/gateway_windows.py::
...
Iterativ has terminal access. Security matters.
...
2. **Test manually**: Run `iterativ` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/IterativKnowledge/iterativ-agent/issues)
- Include: OS, Python version, Iterativ version (`iterativ version`), full error traceback
...
- **Discord**: [discord.gg/IterativKnowledge](https://discord.gg/IterativKnowledge) — for questions, showcasing projects, and sharing skills
