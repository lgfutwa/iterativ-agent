...
Uses a file-based lock (~/.iterativ/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `iterativ update` reloads
# the module) fail with ModuleNotFoundError for iterativ_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from iterativ_constants import get_iterativ_home
from iterativ_cli.config import load_config, _expand_env_vars
from iterativ_time import now as _iterativ_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``iterativ tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from iterativ_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_iterativ_home: Path | None = None


def _get_iterativ_home() -> Path:
    """Resolve Iterativ home dynamically while preserving test monkeypatch hooks."""
    return _iterativ_home or get_iterativ_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    iterativ_home = _get_iterativ_home()
    lock_dir = iterativ_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Iterativ profile.
    ...
    the scheduler's test/override hook and a context-local Iterativ home override
    both point at the resolved profile directory so _get_iterativ_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_iterativ_home() callers agree on the same home.
    ...
    global _iterativ_home
    prior_override = _iterativ_home
    env_snapshot = os.environ.copy()

    from iterativ_cli.profiles import normalize_profile_name, resolve_profile_env
    from iterativ_constants import reset_iterativ_home_override, set_iterativ_home_override
    ...
    override_token = None
    try:
        override_token = set_iterativ_home_override(profile_home)
        _iterativ_home = profile_home
        logger.info(
            "Job '%s': using Iterativ profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _iterativ_home = prior_override
        if override_token is not None:
            reset_iterativ_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ITERATIV_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ITERATIV_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ITERATIV_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ITERATIV_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_iterativ_home() / "scripts"
    ...
    # escape — scripts MUST reside within ITERATIV_HOME/scripts/.
    try:
    ...
    run_env["ITERATIV_HOME"] = str(_get_iterativ_home())
    try:
        from iterativ_constants import get_subprocess_home
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    for skill_name in skill_names:
        ...
        if loaded.get("knowledge_signature"):
            try:
                from knowledge.signatures import format_knowledge_context_block

                knowledge_context_block = format_knowledge_context_block(
                    loaded.get("knowledge_signature"),
                    loaded.get("knowledge_context"),
                )
            except Exception:
                knowledge_context_block = ""
        if parts:
        ...
        if knowledge_context_block:
            parts.extend(["", knowledge_context_block])
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _iterativ_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_iterativ_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_iterativ_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["ITERATIV_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed ITERATIV_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ITERATIV_SESSION_PLATFORM
    #     and ITERATIV_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ITERATIV_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ITERATIV_SESSION_PLATFORM == "telegram".
    ...
    #     ITERATIV_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ITERATIV_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ITERATIV_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ITERATIV_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "ITERATIV_CRON_AUTO_DELIVER_PLATFORM",
        "ITERATIV_CRON_AUTO_DELIVER_CHAT_ID",
        "ITERATIV_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_iterativ_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_iterativ_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["ITERATIV_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ITERATIV_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ITERATIV_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("ITERATIV_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_iterativ_home() / "config.yaml")
        ...
        prefill_file = os.getenv("ITERATIV_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_iterativ_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject ITERATIV_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ITERATIV_HOME. When a workdir is configured, also inject project
        ...
        # override via ITERATIV_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ITERATIV_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ITERATIV_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_iterativ_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _iterativ_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _iterativ_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ITERATIV_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ITERATIV_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ITERATIV_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # Iterativ home override, scheduler _iterativ_home hook, and temporary
