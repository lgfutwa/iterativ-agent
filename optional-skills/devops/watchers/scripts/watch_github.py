...
Usage (via cron with --no-agent):
    ...
    iterativ cron create iterativ-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HERMES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name iterativ-issues --repo IterativKnowledge/iterativ-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.iterativ/.env to avoid the 60 req/hr
