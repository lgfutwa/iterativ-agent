#!/usr/bin/env python3
"""
SQLite State Store for Iterativ Agent.
...
DEFAULT_DB_PATH = get_iterativ_home() / "state.db"
...
# iterativ_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``iterativ_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see iterativ_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple iterativ processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``iterativ_state._set_last_init_error(None)`` explicitly.
    ...
    def get_knowledge_graph(self):
        """Return the profile/session-scoped Knowledge II graph store.

        The graph lives next to the session DB by default (``state.knowledge.db``)
        so tests and alternate profile homes stay isolated without requiring
        another path setting.
        """
        if self._knowledge_graph is None:
            from knowledge.graph.sqlite import SQLiteKnowledgeGraph

            graph_path = self.db_path.with_suffix(".knowledge.db")
            self._knowledge_graph = SQLiteKnowledgeGraph(graph_path)
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Iterativ, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Iterativ session.

        A Iterativ session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Iterativ session is already bound to any Telegram DM topic.
