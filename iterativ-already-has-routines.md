# Iterativ Agent Has Had "Routines" Since March
...
Iterativ equivalent — works today:
```bash
iterativ cron create "0 2 * * *" \
...
Iterativ equivalent — works today:
```bash
iterativ webhook subscribe auth-watch \
  --events "pull_request" \
...
Iterativ equivalent — works today:
```bash
iterativ webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Iterativ implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Iterativ Agent |
|---|---|---|
...
## Things Iterativ Does That Routines Can't
...
Iterativ has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Iterativ Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install iterativ-agent
iterativ setup
```

Set up a scheduled task in 30 seconds:
```bash
iterativ cron create "0 9 * * 1" \
...
```bash
iterativ gateway setup    # enable webhooks
iterativ webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [iterativ-agent.nousresearch.com/docs/guides/automation-templates](https://iterativ-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [iterativ-agent.nousresearch.com](https://iterativ-agent.nousresearch.com)

GitHub: [github.com/IterativKnowledge/iterativ-agent](https://github.com/IterativKnowledge/iterativ-agent)

---

*Iterativ Agent is built by [Iterativ Knowledge](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
