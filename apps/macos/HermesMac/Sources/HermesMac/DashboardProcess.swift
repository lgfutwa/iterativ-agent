...
final class DashboardProcess: ObservableObject {
    ...
    func start() {
        ...
        isStopping = false
        state = .starting
        statusMessage = "Starting the local Hermes dashboard..."
        url = nil
        ...
        let command = HermesCommand.resolve()
        ...
        if let root = command.workingDirectory?.path {
            environment["HERMES_REPO_ROOT"] = root
        }
        ...
        proc.terminationHandler = { [weak self] process in
            DispatchQueue.main.async {
                ...
                if self.state != .running {
                    self.fail("Hermes exited with status \(process.terminationStatus).")
                } else {
                    self.state = .stopped
                    self.statusMessage = "Hermes stopped."
                    self.url = nil
                }
            }
        }
        ...
        do {
            ...
            fail("Could not launch Hermes: \(error.localizedDescription)")
        }
    }
        ...
        readinessTask = Task { [weak self] in
            ...
            for attempt in 0..<240 {
                ...
                if await self.probe(statusURL) {
                    await MainActor.run {
                        self.state = .running
                        self.statusMessage = "Connected to local Hermes."
                ...
                if attempt == 20 {
                    await MainActor.run {
                        self.statusMessage = "Hermes is still starting. First launch may build web assets."
                    }
                }
            ...
            await MainActor.run {
                self.fail("Timed out waiting for Hermes at \(dashboardURL.absoluteString).")
            }
        }
    }
