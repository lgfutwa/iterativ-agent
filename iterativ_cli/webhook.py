"""iterativ webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    iterativ webhook subscribe <name> [options]
    iterativ webhook list
    iterativ webhook remove <name>
    iterativ webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.iterativ/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _iterativ_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_iterativ_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     iterativ gateway setup
  ...
  Then start the gateway: iterativ gateway run
"""
...
def webhook_command(args):
    """Entry point for 'iterativ webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: iterativ webhook {subscribe|list|remove|test}")
        print("Run 'iterativ webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (iterativ gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: iterativ webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from iterativ webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (iterativ gateway run)")
