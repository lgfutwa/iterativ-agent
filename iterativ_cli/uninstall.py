"""
Iterativ Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.iterativ/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Iterativ PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing iterativ-agent or iterativ PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Iterativ Agent" comment and following line
                if '# Iterativ Agent' in line or '# iterativ-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('iterativ' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing iterativ
                if 'iterativ' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the iterativ wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "iterativ",
        Path("/usr/local/bin/iterativ"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains iterativ_cli reference)
                content = wrapper.read_text()
                if 'iterativ_cli' in content or 'iterativ-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``iterativ gateway run`` processes
    ...
    try:
        from iterativ_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from iterativ_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ITERATIV_HOME`` and ``ITERATIV_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\iterativ\git\cmd``, ``%LOCALAPPDATA%\iterativ\git\bin``,
#      ``%LOCALAPPDATA%\iterativ\git\usr\bin``, ``%LOCALAPPDATA%\iterativ\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\iterativ\git\`` and Node to
#      ``%LOCALAPPDATA%\iterativ\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``iterativ dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\iterativ\gateway-service\`` and sometimes
...
def _iterativ_path_markers(iterativ_home: Path) -> list[str]:
    """Path-entry substrings that identify Iterativ-owned User-PATH entries."""
    root = str(iterativ_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare iterativ-agent install dir.
    markers = [root + "\\iterativ-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ITERATIV_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "iterativ".  We don't want to catch
    # unrelated entries like "citerativ-foo" or "ephermeral", so we look for
    # backslash-iterativ as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(iterativ_home: Path) -> list[str]:
    """Strip Iterativ-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _iterativ_path_markers(iterativ_home)
            kept: list[str] = []
...
def remove_iterativ_env_vars_windows() -> list[str]:
    """Delete ITERATIV_HOME and ITERATIV_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ITERATIV_HOME", "ITERATIV_GIT_BASH_PATH"):
                try:
...
def _is_default_iterativ_home(iterativ_home: Path) -> bool:
    """Return True when ``iterativ_home`` points at the default (non-profile) root."""
    try:
        from iterativ_constants import get_default_iterativ_root
        return iterativ_home.resolve() == get_default_iterativ_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ITERATIV_HOME directory.

    We shell out to ``iterativ -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ITERATIV_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m iterativ_cli.main` so we don't depend on a `iterativ`
    #    wrapper that may be half-removed mid-uninstall.
    iterativ_invocation = [_sys.executable, "-m", "iterativ_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                iterativ_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ITERATIV_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.iterativ/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.iterativ/ for future reinstall
    """
    project_root = get_project_root()
    iterativ_home = get_iterativ_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ITERATIV_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_iterativ_home(iterativ_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Iterativ Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {iterativ_home / 'config.yaml'}")
    print(f"  Secrets: {iterativ_home / '.env'}")
    print(f"  Data:    {iterativ_home / 'cron/'}, {iterativ_home / 'sessions/'}, {iterativ_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ITERATIV_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Iterativ data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Iterativ code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in iterativ_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\iterativ\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(iterativ_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Iterativ-owned PATH entries in User environment")

        log_info("Removing ITERATIV_HOME / ITERATIV_GIT_BASH_PATH User env vars...")
        removed_env = remove_iterativ_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Iterativ-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing iterativ command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.iterativ/, just remove the iterativ-agent subdir
            if iterativ_home in project_root.parents or project_root.parent == iterativ_home:
    ...
    #     under ITERATIV_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(iterativ_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(iterativ_home)
    ...
    # 5. Optionally remove ~/.iterativ/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ITERATIV_HOME dirs live under
        ...
        try:
            if iterativ_home.exists():
                shutil.rmtree(iterativ_home)
                log_success(f"Removed {iterativ_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {iterativ_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {iterativ_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {iterativ_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/IterativKnowledge/iterativ-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/IterativKnowledge/iterativ-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Iterativ Agent! ⚕")
    print()
