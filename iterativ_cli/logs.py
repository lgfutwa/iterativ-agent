"""``iterativ logs`` — view and filter Iterativ log files.
...
Usage examples::
    ...
    iterativ logs                    # last 50 lines of agent.log
    iterativ logs -f                 # follow agent.log in real time
    iterativ logs errors             # last 50 lines of errors.log
    iterativ logs gateway -n 100    # last 100 lines of gateway.log
    iterativ logs --level WARNING    # only WARNING+ lines
    iterativ logs --session abc123   # filter by session ID substring
    iterativ logs --component tools  # only tool-related lines
    iterativ logs --since 1h         # lines from the last hour
    iterativ logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_iterativ_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Iterativ runs — try 'iterativ chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_iterativ_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_iterativ_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_iterativ_home()}/logs/")
        return

    print(f"Log files in {display_iterativ_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'iterativ chat' to generate logs)")
