"""`iterativ checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.iterativ/checkpoints/``.  Actions:

    iterativ checkpoints               # same as `status`
    iterativ checkpoints status        # total size, project count, breakdown
    iterativ checkpoints list          # per-project checkpoint counts + workdir
    iterativ checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    iterativ checkpoints clear [-f]    # nuke the entire base (asks first)
    iterativ checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    iterativ checkpoints
    iterativ checkpoints prune --retention-days 3 --max-size-mb 200
    iterativ checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: iterativ checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``iterativ checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `iterativ checkpoints` → status
