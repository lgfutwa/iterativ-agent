...
# xAI's public API is OpenAI-compatible for the endpoints Iterativ commonly
# uses. The Responses endpoint is included because Iterativ' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Iterativ-managed OAuth credentials."""

    auth_hint = "iterativ auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`iterativ auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`iterativ auth reset xai-oauth` or re-authenticate with "
                    "`iterativ auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `iterativ auth add xai-oauth --type oauth`."
            )
