"""CLI handlers for the ``iterativ proxy`` subcommand."""
...
from iterativ_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "iterativ proxy requires aiohttp. Install one of:\n"
        "  pip install 'iterativ-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"iterativ login {adapter.name}")
        print(
    ...
    print(
        f"Starting Iterativ proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Iterativ proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: iterativ proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``iterativ proxy <subcommand>``."""
    ...
    print(
        "iterativ proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  iterativ proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  iterativ proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  iterativ proxy providers\n"
