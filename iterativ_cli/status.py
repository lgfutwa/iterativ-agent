"""
Status command for iterativ CLI.

Shows the status of all Iterativ Agent components.
"""
...
from iterativ_cli.auth import AuthError, resolve_provider
from iterativ_cli.colors import Colors, color
from iterativ_cli.config import get_env_path, get_env_value, get_iterativ_home, load_config
from iterativ_cli.models import provider_label
from iterativ_cli.nous_subscription import get_nous_subscription_features
from iterativ_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``iterativ config``'s
...
def show_status(args):
    """Show status of all Iterativ Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Iterativ Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: iterativ auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: iterativ model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: iterativ auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: iterativ auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: iterativ model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from iterativ_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'iterativ-agent[vercel]')"
    ...
    try:
        from iterativ_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   iterativ gateway")
    ...
    jobs_file = get_iterativ_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_iterativ_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'iterativ doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'iterativ setup' to configure", Colors.DIM))
    print()
