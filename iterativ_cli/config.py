"""
Configuration management for Iterativ Agent.

Config files are stored in ~/.iterativ/ for easy access:
- ~/.iterativ/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.iterativ/.env         - API keys and secrets

This module provides:
- iterativ config          - Show current configuration
- iterativ config edit     - Open config in editor
- iterativ config set      - Set a specific value
- iterativ config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.iterativ/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``iterativ logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  iterativ config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `iterativ plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "ITERATIV_LANGFUSE_ENV",
    "ITERATIV_LANGFUSE_RELEASE",
    "ITERATIV_LANGFUSE_SAMPLE_RATE",
    "ITERATIV_LANGFUSE_MAX_CHARS",
    "ITERATIV_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from iterativ_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ITERATIV_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_iterativ_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Iterativ is running in package-manager-managed mode.

    Two signals: the ITERATIV_MANAGED env var (set by the systemd service),
    or a .managed marker file in ITERATIV_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade iterativ-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Iterativ was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.iterativ/.install_method`` file (written by installers)
    2. ITERATIV_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_iterativ_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.iterativ/.install_method."""
    stamp = get_iterativ_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade iterativ-agent"
    if method == "docker":
        return "docker pull nousresearch/iterativ-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade iterativ-agent"
        return "pip install --upgrade iterativ-agent"
    return "iterativ update"
...
def format_managed_message(action: str = "modify this Iterativ installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ITERATIV_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Iterativ installation is managed by NixOS "
            f"(ITERATIV_MANAGED={env_hint}).\n"
            "Edit services.iterativ-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Iterativ installation is managed by Homebrew "
            f"(ITERATIV_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade iterativ-agent"
        )

    return (
        f"Cannot {action}: this Iterativ installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Iterativ."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ITERATIV_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, iterativ_bin
    or None if container mode is not active, we're already inside the
    container, or ITERATIV_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ITERATIV_DEV") == "1":
        return None
    ...
    container_mode_file = get_iterativ_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "iterativ-agent")
    exec_user = info.get("exec_user", "iterativ")
    iterativ_bin = info.get("iterativ_bin", "/data/current-package/bin/iterativ")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "iterativ_bin": iterativ_bin,
    }
...
# Re-export from iterativ_constants — canonical definition lives there.
from iterativ_constants import get_iterativ_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_iterativ_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_iterativ_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the iterativ group can
    share state with the gateway service.

    The mode can be overridden via the ITERATIV_HOME_MODE environment variable
    (e.g. ITERATIV_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ITERATIV_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ITERATIV_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Iterativ runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ITERATIV_CONTAINER") or os.environ.get("ITERATIV_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ITERATIV_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ITERATIV_HOME if the user doesn't have one yet."""
...
def ensure_iterativ_home():
    """Ensure ~/.iterativ directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_iterativ_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_iterativ_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ITERATIV_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Iterativ-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Iterativ auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Iterativ sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Iterativ
        ...
        #  "/home/user/.iterativ/cache/documents:/output"]
        ...
        # bundled Iterativ image, which drops to the `iterativ` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Iterativ sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Iterativ should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``iterativ chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.iterativ/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: iterativ sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Iterativ truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``iterativ kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``iterativ kanban decompose`` and the kanban
        ...
        # ``iterativ profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `iterativ model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `iterativ --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `iterativ -c` muscle memory.  Default off so existing
        # users aren't surprised.  ITERATIV_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/iterativ`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.iterativ/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.iterativ/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # Knowledge II substrate -- explicit graph/provenance layer under memory,
    # skills, tools, and synthesis. Soft mode records warnings without blocking
    # classic agent behavior; hard mode is opt-in per deployment/domain policy.
    "knowledge": {
        "enabled": True,
        # Empty = use the active session DB's sibling graph when available,
        # otherwise ~/.iterativ/knowledge/graph.db.
        "db_path": "",
        "validation_mode": "soft",  # soft | hard
        "capture_memory_writes": True,
        # Tool hooks only extract claims from explicit structured fields:
        # {"knowledge_claims": [...]} or {"knowledge_claim": ...}.
        "capture_tool_results": True,
        "source_credibility_threshold": 0.25,
    ...
    # response. If not, Iterativ feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Iterativ auto-pauses the goal and
    ...
    # always goes to ~/.iterativ/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ITERATIV_SKILL_DIR} and ${ITERATIV_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `iterativ curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.iterativ/skills/ into
        # ~/.iterativ/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `iterativ curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for iterativ-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Iterativ Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ITERATIV_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.iterativ/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ITERATIV_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `iterativ doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``iterativ_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Iterativ to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ITERATIV_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `iterativ -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `iterativ` launches with
        ...
        # decomposition is manual via `iterativ kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     iterativ-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.iterativ/logs/.
    ...
    # update model picker lists without shipping a iterativ-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://iterativ-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `iterativ model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.iterativ/state.db.
    ...
    "sessions": {
        ...
        # default of ``iterativ sessions prune``.
        "retention_days": 90,
    ...
    # ``iterativ update`` behaviour.
    "updates": {
        # Run a full ``iterativ backup``-style zip of ITERATIV_HOME before every
        # ``iterativ update``.  Backups land in ``<ITERATIV_HOME>/backups/`` and
        # can be restored with ``iterativ import <path>``.  Off by default —
        # on large ITERATIV_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ITERATIV_HOME>/lsp/bin/`` on first use.
    ...
    # `iterativ tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'iterativ model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Iterativ auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @iterativ:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ITERATIV_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #iterativ)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: iterativ-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'iterativ-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Iterativ API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Iterativ API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Iterativ API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ITERATIV_TOOL_PROGRESS and ITERATIV_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "ITERATIV_TOOL_PROGRESS": {
    ...
    "ITERATIV_PREFILL_MESSAGES_FILE": {
    ...
    "ITERATIV_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.iterativ.config`` entries, then checks
    ...
    try:
        ...
        # should never break `iterativ update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Iterativ versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``iterativ_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_KNOWN_ROOT_KEYS = {
    ...
    "auxiliary", "custom_providers", "context", "memory", "knowledge", "gateway",
    "sessions",
}
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'iterativ setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Iterativ won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'iterativ doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ITERATIV_HOME", "~/.iterativ")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ITERATIV_TOOL_PROGRESS")
            old_mode = get_env_value("ITERATIV_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ITERATIV_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ITERATIV_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ITERATIV_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ITERATIV_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ITERATIV_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_iterativ_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `iterativ plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `iterativ curator status` has no
    ...
    #   3. Creates `~/.iterativ/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_iterativ_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_iterativ_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: iterativ config set <key> <value>")
    ...
    # metadata.iterativ.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _iterativ_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.iterativ/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.iterativ/config.yaml.
    ...
    (which change ``ITERATIV_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_iterativ_home()
        config_path = get_config_path()
        path_key = str(config_path)
...
#   openai-codex (OAuth — iterativ auth) — OpenAI Codex
#   nous         (OAuth — iterativ auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.iterativ/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.iterativ/.env.
    ...
    interactive menu render (`iterativ tools`, `iterativ setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `iterativ tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Iterativ env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.iterativ/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.iterativ/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.iterativ/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.iterativ/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Iterativ — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Iterativ vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.iterativ/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Iterativ Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  iterativ config edit     # Edit config file", Colors.DIM))
    print(color("  iterativ config set <key> <value>", Colors.DIM))
    print(color("  iterativ setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: iterativ config set <key> <value>")
            print()
            print("Examples:")
            print("  iterativ config set model anthropic/claude-sonnet-4")
            print("  iterativ config set terminal.backend docker")
            print("  iterativ config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'iterativ config migrate' to add them")
        ...
        print("  iterativ config           Show current configuration")
        print("  iterativ config edit      Open config in editor")
        print("  iterativ config set <key> <value>   Set a config value")
        print("  iterativ config check     Check for missing/outdated config")
        print("  iterativ config migrate   Update config with new options")
        print("  iterativ config path      Show config file path")
        print("  iterativ config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``iterativ config`` UI so users
