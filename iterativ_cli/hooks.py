"""iterativ hooks — inspect and manage shell-script hooks.

Usage::

    iterativ hooks list
    iterativ hooks test <event> [--for-tool X] [--payload-file F]
    iterativ hooks revoke <command>
    iterativ hooks doctor

Consent records live under ``~/.iterativ/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.iterativ/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``iterativ hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: iterativ hooks {list|test|revoke|doctor}")
        print("Run 'iterativ hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.iterativ/config.yaml.")
        print("See `iterativ hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `iterativ hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `iterativ hooks test` and `iterativ hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Iterativ wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `iterativ hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `iterativ hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `iterativ hooks doctor`.")
