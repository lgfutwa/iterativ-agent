"""
Dump command for iterativ CLI.

Outputs a compact, plain-text summary of the user's Iterativ setup
...
from iterativ_cli.config import get_iterativ_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``iterativ dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from iterativ_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(iterativ_home: Path) -> int:
...
def _cron_summary(iterativ_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = iterativ_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"iterativ_home:      {display_iterativ_home()}")
    ...
    toolsets = config.get("toolsets", ["iterativ-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(iterativ_home)}")
    lines.append(f"  skills:             {_count_skills(iterativ_home)}")
