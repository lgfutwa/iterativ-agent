"""Profile distributions — shareable, packaged Iterativ profiles via git.

A distribution is a Iterativ profile published as a git repository (or
...
* ``iterativ profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``iterativ skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``iterativ profile``, not a parallel tree):

    iterativ profile install <source> [--name N] [--alias] [--force] [--yes]
    iterativ profile update  <name>  [--force-config] [--yes]
    iterativ profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "iterativ_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".iterativ_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "iterativ-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            iterativ_requires=str(data.get("iterativ_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.iterativ_requires:
            out["iterativ_requires"] = self.iterativ_requires
        if self.author:
...
def check_iterativ_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Iterativ {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Iterativ distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``iterativ profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Iterativ profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Iterativ distribution."
        )

    # Version check up-front so we fail fast
    check_iterativ_requires(manifest.iterativ_requires, iterativ_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.iterativ).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="iterativ_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `iterativ profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`iterativ profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="iterativ_dist_update_") as tmp:
        plan = plan_install(
