"""
Gateway subcommand for iterativ CLI.

Handles: iterativ gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_iterativ_home is imported lazily at call sites to avoid ImportError
# when iterativ_constants is cached from a pre-update version during `iterativ update`.
from iterativ_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from iterativ_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "iterativ-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "iterativ CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``iterativ gateway status`` from falsely counting the ``iterativ`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``iterativ gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "iterativ_cli.main gateway",
        "iterativ_cli.main --profile",
        "iterativ_cli.main -p",
        "iterativ_cli/main.py gateway",
        "iterativ_cli/main.py --profile",
        "iterativ_cli/main.py -p",
        "iterativ gateway",
        "gateway/run.py",
    ]
    current_home = str(get_iterativ_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ITERATIV_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ITERATIV_HOME
        ...
        # ITERATIV_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "ITERATIV_HOME=" in command and f"ITERATIV_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``iterativ update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Iterativ profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Iterativ profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from iterativ_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "iterativ_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``iterativ update`` users
    # with no running gateway until they re-invoke ``iterativ gateway``
...
def _sync_iterativ_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ITERATIV_HOME``.

    Under ``sudo``, ``ITERATIV_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_iterativ_home` falls back to ``/root/.iterativ`` — the wrong
    profile. The unit file pins ``ITERATIV_HOME`` for the actual gateway
    ...
    unit_home = env.get("ITERATIV_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ITERATIV_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ITERATIV_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}iterativ gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}iterativ gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from iterativ_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `iterativ gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``iterativ gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from iterativ_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ITERATIV_HOME-scoped).
...
from iterativ_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``iterativ gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ITERATIV_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ITERATIV_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "iterativ-gateway"
SERVICE_DESCRIPTION = "Iterativ Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ITERATIV_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ITERATIV_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from iterativ_constants import get_default_iterativ_root
    home = get_iterativ_home().resolve()
    default = get_default_iterativ_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ITERATIV_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(iterativ_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ITERATIV_HOME is a named profile.

    For ``~/.iterativ/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        iterativ_home: Optional explicit ITERATIV_HOME path. Defaults to the current
            ``get_iterativ_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from iterativ_constants import get_default_iterativ_root
    home = Path(iterativ_home or str(get_iterativ_home())).resolve()
    default = get_default_iterativ_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ITERATIV_HOME.

    Default ``~/.iterativ`` returns ``iterativ-gateway`` (backward compatible).
    Profile ``~/.iterativ/profiles/coder`` returns ``iterativ-gateway-coder``.
    Any other ITERATIV_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Iterativ installs that predate the
# iterativ-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (iterativ-gateway-*.service) and unrelated third-party
# "iterativ" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("iterativ.service",)
...
def _find_legacy_iterativ_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Iterativ gateway units.

    Detects unit files installed by older Iterativ versions that used a
    different service name (e.g. ``iterativ.service`` before the rename to
    ``iterativ-gateway.service``). When both a legacy unit and the current
    ``iterativ-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``iterativ-gateway-coder.service`` and unrelated third-party
      ``iterativ-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``iterativ.service`` running an unrelated
...
def has_legacy_iterativ_units() -> bool:
    """Return True when any legacy Iterativ gateway unit files exist."""
    return bool(_find_legacy_iterativ_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Iterativ gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_iterativ_units()
    if not legacy:
        return
    print_warning("Legacy Iterativ gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current iterativ-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    iterativ gateway migrate-legacy")


def remove_legacy_iterativ_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Iterativ gateway unit files.

    Iterates over whatever ``_find_legacy_iterativ_units()`` returns — which is
    ...
    legacy = _find_legacy_iterativ_units()
    if not legacy:
        print("No legacy Iterativ gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Iterativ gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: iterativ gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo iterativ gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    iterativ gateway uninstall")
    print_info("    sudo iterativ gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Iterativ can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo iterativ gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo iterativ gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo iterativ gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Iterativ often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.iterativ`` → ``ai.iterativ.gateway.plist`` (backward compatible).
    Profile ``~/.iterativ/profiles/coder`` → ``ai.iterativ.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.iterativ.gateway-{suffix}" if suffix else "ai.iterativ.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.iterativ/iterativ-agent  -> /home/alice/.iterativ/iterativ-agent
      /opt/iterativ                 -> /opt/iterativ  (kept as-is)
...
def _iterativ_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ITERATIV_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_iterativ_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.iterativ                    → /home/alice/.iterativ
      /root/.iterativ/profiles/coder     → /home/alice/.iterativ/profiles/coder
      /opt/custom-iterativ               → /opt/custom-iterativ  (kept as-is)
    """
    current_iterativ = get_iterativ_home().resolve()
    current_default = (Path.home() / ".iterativ").resolve()
    target_default = Path(target_home_dir) / ".iterativ"

    # Default ~/.iterativ → remap to target user's default
    if current_iterativ == current_default:
        return str(target_default)

    # Profile or subdir of ~/.iterativ → preserve the relative structure
    try:
        relative = current_iterativ.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.iterativ) — keep as-is
        return str(current_iterativ)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    iterativ_home = get_iterativ_home()
    iterativ_node = iterativ_home / "node" / "bin"
    if _is_dir(iterativ_node):
        candidates.append(str(iterativ_node))
    iterativ_nm = iterativ_home / "node_modules" / ".bin"
    if _is_dir(iterativ_nm):
        candidates.append(str(iterativ_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        iterativ_home = _iterativ_home_for_target_user(home_dir)
        profile_arg = _profile_arg(iterativ_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m iterativ_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ITERATIV_HOME={iterativ_home}"
Restart=always
RestartSec=5
...
"""
    ...
    iterativ_home = str(get_iterativ_home().resolve())
    profile_arg = _profile_arg(iterativ_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ITERATIV_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ITERATIV_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../iterativ_test`` path into
    # ``Environment="ITERATIV_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Iterativ install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo iterativ gateway uninstall --system")
    print_info("         iterativ gateway install")
    print_info("         iterativ gateway start")
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (iterativ.service from pre-rename installs)
    # before installing the new iterativ-gateway.service. If both remain, they
    ...
    if has_legacy_iterativ_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_iterativ_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}iterativ gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}iterativ gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}iterativ gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_iterativ_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `iterativ gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `iterativ gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}iterativ gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}iterativ gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}iterativ gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}iterativ gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.iterativ.gateway-{suffix}" if suffix else "ai.iterativ.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    iterativ_home = str(get_iterativ_home().resolve())
    log_dir = get_iterativ_home() / "logs"
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>iterativ_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>ITERATIV_HOME</key>
        <string>{iterativ_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Iterativ install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  iterativ gateway status             # Check status")
    from iterativ_constants import display_iterativ_home as _dhh
...
def launchd_stop():
    ...
    # `iterativ gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Iterativ install")
    else:
        print("⚠ Service definition is stale relative to the current Iterativ install")
        print("  Run: iterativ gateway start")
    ...
    if deep:
        log_file = get_iterativ_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ITERATIV_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Iterativ gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'iterativ' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/iterativ/docker/entrypoint.sh before the Iterativ command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ITERATIV_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set ITERATIV_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `iterativ gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ITERATIV_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `iterativ gateway restart` which already
    ...
    # `iterativ gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Iterativ Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # ITERATIV_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ITERATIV_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Iterativ will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @iterativ:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. iterativ) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Iterativ delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Iterativ agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Iterativ will use (e.g., iterativ@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Iterativ connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: iterativ pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Iterativ will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``iterativ setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run iterativ under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.iterativ/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_iterativ_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'iterativ pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: iterativ pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'iterativ gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from iterativ_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from iterativ_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from iterativ_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Iterativ will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Iterativ will store the returned account_id/token in ~/.iterativ/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `iterativ gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_iterativ_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `iterativ pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `iterativ pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"IterativAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``iterativ_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from iterativ_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.iterativ/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.iterativ/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: iterativ gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: iterativ gateway install")
                else:
                    print_info("  You can install later: iterativ gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo iterativ gateway install --system")
                    print_info("  Or run in foreground:  iterativ gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: iterativ gateway run")
                print_info("  For persistence:   tmux new -s iterativ 'iterativ gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from iterativ_constants import display_iterativ_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: iterativ gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup iterativ gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: iterativ gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'iterativ gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``iterativ gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: iterativ gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: iterativ gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s iterativ 'iterativ gateway run'")
                print()
            ...
            print()
            print("  iterativ gateway run                              # direct foreground")
            print("  tmux new -s iterativ 'iterativ gateway run'         # persistent via tmux")
            print("  nohup iterativ gateway run > ~/.iterativ/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: iterativ gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: iterativ gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: iterativ gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: iterativ gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    iterativ gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: iterativ gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  iterativ gateway install")
                else:
                    print("To install as a service:")
                    print("  iterativ gateway install")
                    print("  sudo iterativ gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  iterativ gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup iterativ gateway run > ~/.iterativ/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s iterativ 'iterativ gateway run'         # persistent via tmux")
                    print("  nohup iterativ gateway run > ~/.iterativ/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  iterativ gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  iterativ gateway install  # Install as user service")
                    print("  sudo iterativ gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Iterativ gateway unit files from
        # pre-rename installs (e.g. iterativ.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_iterativ_units(interactive=not yes, dry_run=dry_run)
