"""Shell completion script generation for iterativ CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_iterativ_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Iterativ Agent bash completion
# Add to ~/.bashrc:
#   eval "$(iterativ completion bash)"

_iterativ_profiles() {{
    local profiles_dir="$HOME/.iterativ/profiles"
...
_iterativ_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_iterativ_profiles)" -- "$cur"))
        return
    fi
...
complete -F _iterativ_completion iterativ
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef iterativ
# Iterativ Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(iterativ completion zsh)"

_iterativ_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.iterativ/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.iterativ/profiles 2>/dev/null)}}")
    fi
...
_iterativ() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_iterativ_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'iterativ command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Iterativ Agent fish completion",
        "# Add to your config:",
        "#   iterativ completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __iterativ_profiles",
        "    echo default",
        "    if test -d $HOME/.iterativ/profiles",
        "        ls $HOME/.iterativ/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c iterativ -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c iterativ -f -s p -l profile"
        " -d 'Profile name' -xa '(__iterativ_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__iterativ_profiles)' -d 'Profile name'"
                )
