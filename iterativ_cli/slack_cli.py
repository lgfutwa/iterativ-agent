"""``iterativ slack ...`` CLI subcommands.

Today only ``iterativ slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ iterativ slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Iterativ. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Iterativ deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from iterativ_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Iterativ in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``iterativ_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ITERATIV_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Iterativ")
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ITERATIV_HOME") or str(Path.home() / ".iterativ")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Iterativ app\n"
