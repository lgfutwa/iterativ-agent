...
on Japanese locales, etc.).  Iterativ's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``iterativ_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Iterativ is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ITERATIV_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ITERATIV_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Iterativ-managed Git install directories so
    ...
    # already-running shells don't see the change — which means iterativ
    ...
    # install without Iterativ-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Iterativ's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Iterativ's
    ``iterativ config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Iterativ (User env var in Windows Settings, or
    export in a PowerShell profile) and Iterativ picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Iterativ-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\iterativ\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``iterativ`` from right after install) retain their old PATH.

    Any subprocess Iterativ spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Iterativ-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "iterativ", "git", "cmd"),
        os.path.join(local_appdata, "iterativ", "git", "bin"),
        os.path.join(local_appdata, "iterativ", "git", "usr", "bin"),
        # Iterativ venv Scripts directory — host of the iterativ.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes iterativ, but harmless to include.
        os.path.join(local_appdata, "iterativ", "iterativ-agent", "venv", "Scripts"),
