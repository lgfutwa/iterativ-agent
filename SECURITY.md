...
Report privately via [GitHub Security Advisories](https://github.com/IterativKnowledge/iterativ-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Iterativ Agent does not operate a bug
bounty program.**
...
- Environment details (`iterativ version`, commit SHA, OS, Python
  version).
...
Iterativ Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Iterativ Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Iterativ Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Iterativ Agent's documentation
...
Iterativ Agent supports two OS-level isolation postures. They address
...
Iterativ Agent supports this in two ways:

- **Iterativ Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Iterativ Agent's in-process heuristics
...
Iterativ Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Iterativ Agent
itself. Bugs in Iterativ Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Iterativ Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Iterativ Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Iterativ Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  iterativ-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Iterativ Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Iterativ Agent. Bugs in Iterativ
...
- Iterativ Agent includes supply-chain guards for MCP server
