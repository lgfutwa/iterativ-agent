...
export function ChatSidebar({ channel, className }: ChatSidebarProps) {
  ...
  // /api/events in iterativ_cli/web_server.py for the broadcast hop.
  //
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${proto}//${window.location.host}${ITERATIV_BASE_PATH}/api/events?${qs.toString()}`,
    );
  ...
  return (
    <aside
      className={cn(
        "flex h-full w-full min-w-0 shrink-0 flex-col gap-3 overflow-y-auto overflow-x-hidden pr-1 normal-case text-[var(--studio-text)] lg:w-80",
        className,
      )}
    >
      <Card className="flex items-center justify-between gap-2 rounded-[12px] border-[var(--studio-border-subtle)] bg-white/90 px-3 py-2 text-[var(--studio-text)] shadow-none">
        <div className="min-w-0">
          <div className="text-xs font-medium uppercase tracking-normal text-[var(--studio-text-muted)]">
            model
          </div>
          ...
          <Button
            ...
            className="self-start min-w-0 px-0 py-0 normal-case tracking-normal text-sm font-medium text-[var(--studio-text)] hover:underline disabled:no-underline"
      ...
      {banner && (
        <Card className="flex items-start gap-2 rounded-[12px] border-destructive/40 bg-destructive/5 px-3 py-2 text-xs text-[var(--studio-text)] shadow-none">
      ...
      <Card className="flex min-h-0 flex-none flex-col rounded-[12px] border-[var(--studio-border-subtle)] bg-white/90 px-2 py-2 text-[var(--studio-text)] shadow-none">
        <div className="px-1 pb-2 text-xs font-medium uppercase tracking-normal text-[var(--studio-text-muted)]">
          tools
        </div>

        <div className="flex min-h-0 flex-col gap-1.5">
          {tools.length === 0 ? (
            <div className="px-2 py-4 text-center text-xs text-[var(--studio-text-muted)]">
              no tool calls yet
            </div>
          ) : (
