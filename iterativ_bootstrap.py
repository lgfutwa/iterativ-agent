"""Windows UTF-8 bootstrap for Iterativ entry points.
...
should be imported at the very top of every Iterativ entry point
(``iterativ``, ``iterativ-agent``, ``iterativ-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import iterativ_bootstrap``
# (or ``from iterativ_bootstrap import apply_windows_utf8_bootstrap``) at
