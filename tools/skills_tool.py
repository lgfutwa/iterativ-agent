...
# All skills live in ~/.iterativ/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ITERATIV_HOME/.env."""
    env_path = get_iterativ_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("ITERATIV_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("ITERATIV_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_iterativ_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.iterativ/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _build_knowledge_fields(
    ...
    *,
    ...
    """Return normalized Knowledge II signature and graph context for a skill."""
    try:
        from knowledge.signatures import (
            build_skill_knowledge_context,
            normalize_knowledge_signature,
        )

        signature = normalize_knowledge_signature(frontmatter.get("knowledge_signature"))
        if not signature:
            return None, None
        if graph is None:
            try:
                ...
                graph = get_knowledge_graph_from_config()
            except Exception:
                graph = None
        context = build_skill_knowledge_context(
            signature,
            task=skill_name,
            graph=graph,
        )
        ...
        logger.debug("Could not build Knowledge II fields for skill %s", skill_name, exc_info=True)
        return None, None
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ITERATIV_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("ITERATIV_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ITERATIV_PLATFORM`` environment variable
    3. ``ITERATIV_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from iterativ_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ITERATIV_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.iterativ/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``iterativ skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_iterativ_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from iterativ_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: iterativ plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    knowledge_signature, knowledge_context = _build_knowledge_fields(
        parsed_frontmatter,
    ...
    if knowledge_signature:
        result["knowledge_signature"] = knowledge_signature
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.iterativ/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `iterativ skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.iterativ.* first (agentskills.io convention), fall back to top-level
        iterativ_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            iterativ_meta = metadata.get("iterativ", {}) or {}

        tags = _parse_tags(iterativ_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            iterativ_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
