...
Lets the LLM write a Python script that calls Iterativ tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `iterativ_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `iterativ_tools.py` with file-based RPC stubs
...
# available on every platform Iterativ itself runs on.
...
# ---------------------------------------------------------------------------
# iterativ_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_iterativ_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the iterativ_tools.py stub module.
...
"""Auto-generated Iterativ tools RPC stubs."""
...
def _connect():
    ...
    ITERATIV_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ITERATIV_RPC_SOCKET"]
...
"""Auto-generated Iterativ tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ITERATIV_RPC_DIR") or os.path.join(tempfile.gettempdir(), "iterativ_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated iterativ_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/iterativ_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_iterativ_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/iterativ_tools.py", tools_src)
        ...
        env_prefix = (
            f"ITERATIV_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ITERATIV_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with iterativ_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="iterativ_sandbox_")
    ...
    # selector from ITERATIV_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"iterativ_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated iterativ_tools module.
        ...
        tools_src = generate_iterativ_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "iterativ_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ITERATIV_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ITERATIV_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the iterativ-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from iterativ_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _iterativ_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _iterativ_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ITERATIV_TIMEZONE is an internal Iterativ setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ITERATIV_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ITERATIV_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ITERATIV_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.iterativ/.env')).
...
def _load_config() -> dict:
    ...
    ``iterativ --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (iterativ-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``iterativ tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and iterativ-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.iterativ/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Iterativ tools programmatically. "
        ...
        f"Available via `from iterativ_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into iterativ_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from iterativ_tools import {import_str}` "
