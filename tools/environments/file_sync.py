...
_FORCE_SYNC_ENV = "ITERATIV_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.iterativ") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.iterativ to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.iterativ", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ITERATIV_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, iterativ_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.iterativ/`` directory as a tar archive,
        ...
        # against an uninitialized remote .iterativ/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (iterativ_home or get_iterativ_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="iterativ-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.iterativ/skills/a.md`` →
        ``~/.iterativ/skills/a.md``, a new remote file at
        ``/root/.iterativ/skills/b.md`` maps to ``~/.iterativ/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
