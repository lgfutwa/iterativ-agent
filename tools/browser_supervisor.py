...
One ``CDPSupervisor`` runs per Iterativ ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "iterativ-dialog-bridge.invalid"
...
(() => {
  if (window.__iterativDialogBridgeInstalled) return;
  window.__iterativDialogBridgeInstalled = true;
  const ENDPOINT = "http://iterativ-dialog-bridge.invalid/";
