...
it is automatically downloaded from GitHub releases to $ITERATIV_HOME/bin/tirith.
...
def _get_iterativ_home() -> str:
    """Return the Iterativ home directory, respecting ITERATIV_HOME env var."""
    return str(get_iterativ_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_iterativ_home(), ".tirith-install-failed")
...
def _iterativ_bin_dir() -> str:
    """Return $ITERATIV_HOME/bin, creating it if needed."""
    d = os.path.join(_get_iterativ_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ITERATIV_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_iterativ_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ITERATIV_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ITERATIV_HOME/bin/tirith
    ...
    iterativ_bin = os.path.join(_iterativ_bin_dir(), "tirith")
    if os.path.isfile(iterativ_bin) and os.access(iterativ_bin, os.X_OK):
        _resolved_path = iterativ_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return iterativ_bin
