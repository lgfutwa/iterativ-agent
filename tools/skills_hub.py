#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Iterativ Skills Hub.
...
Used by iterativ_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            iterativ_meta = metadata.get("iterativ", {})
            if isinstance(iterativ_meta, dict):
                tags = iterativ_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            iterativ_meta = metadata.get("iterativ", {})
            if isinstance(iterativ_meta, dict):
                raw_tags = iterativ_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Iterativ Knowledge) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.iterativ/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                iterativ_meta = meta_block.get("iterativ", {})
                if isinstance(iterativ_meta, dict):
                    tags = iterativ_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Iterativ centralized index source
# ---------------------------------------------------------------------------

ITERATIV_INDEX_URL = "https://iterativ-agent.nousresearch.com/docs/api/skills-index.json"
ITERATIV_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "iterativ-index.json"
ITERATIV_INDEX_TTL = 6 * 3600  # 6 hours


def _load_iterativ_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ITERATIV_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ITERATIV_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ITERATIV_INDEX_CACHE_FILE.stat().st_mtime
            if age < ITERATIV_INDEX_TTL:
                return json.loads(ITERATIV_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ITERATIV_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Iterativ index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Iterativ index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ITERATIV_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ITERATIV_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class IterativIndexSource(SkillSource):
    """Skill source backed by the centralized Iterativ Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_iterativ_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "iterativ-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "iterativ-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        IterativIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "iterativ-index"
