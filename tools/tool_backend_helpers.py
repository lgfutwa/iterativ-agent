...
def managed_nous_tools_enabled() -> bool:
    ...
    try:
        ...
        from iterativ_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.iterativ/.env`` (via
    ``iterativ_cli.config.get_env_value`` when available) so tool-side
