...
`mcp_tool.py`, and `iterativ_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# IterativMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_iterativ_provider_class() -> Optional[type]:
    ...
    class IterativMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._iterativ_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`IterativTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, IterativTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._iterativ_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._iterativ_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import IterativTokenStorage
                        if isinstance(storage, IterativTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._iterativ_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import IterativTokenStorage
            if not isinstance(storage, IterativTokenStorage):
                return
    ...
    return IterativMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ITERATIV_PROVIDER_CLS: Optional[type] = _make_iterativ_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`IterativMCPOAuthProvider` directly using the helpers
        ...
        storage = IterativTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``iterativ mcp remove <name>`` and (indirectly) by
        ``iterativ mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
