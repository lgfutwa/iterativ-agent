...
Copies bundled skills from the repo's skills/ directory into ~/.iterativ/skills/
...
The manifest lives at ~/.iterativ/skills/.bundled_manifest.
"""
...
from iterativ_constants import get_bundled_skills_dir, get_iterativ_home
...
def _get_bundled_dir() -> Path:
    ...
    Checks ITERATIV_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.iterativ/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.iterativ/skills/ using the manifest.
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `iterativ skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `iterativ skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `iterativ update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.iterativ/skills/ ...")
