...
Backend is selected during ``iterativ tools`` setup (web.backend in config.yaml).
When available, Iterativ can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.iterativ/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``iterativ tools``).
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `iterativ tools` to set one up."
                ),
            }
        else:
            logger.info(
