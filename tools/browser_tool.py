...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    iterativ_node_bin = str(iterativ_home / "node" / "bin")
    iterativ_node_root = str(iterativ_home / "node")
    iterativ_nm_bin = str(iterativ_home / "node_modules" / ".bin")
    return [iterativ_node_bin, iterativ_node_root, iterativ_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.iterativ/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.iterativ/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Iterativ silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Iterativ CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-iterativ_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed iterativ processes — this way every clean iterativ exit sweeps
    ...
    # Sweep orphans from other crashed iterativ processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live iterativ processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current iterativ PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live iterativ process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning iterativ process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced iterativ PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent iterativ instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-iterativ_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live iterativ process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Iterativ-managed
    ...
    # Build an extended search PATH including Iterativ-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                iterativ_nm = str(get_iterativ_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=iterativ_nm)
            if not recheck:
                iterativ_node_bin = str(get_iterativ_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=iterativ_node_bin)
            if not recheck:
                iterativ_node_root = str(get_iterativ_home() / "node")
                recheck = shutil.which("agent-browser", path=iterativ_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this iterativ PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = iterativ_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from iterativ_constants import get_iterativ_dir
    screenshots_dir = get_iterativ_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/iterativ-agent:latest")
                else:
