"""Persistent slash-command worker — one IterativCLI per TUI session.
...
def _run(cli: IterativCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ITERATIV_SESSION_KEY"] = args.session_key
    os.environ["ITERATIV_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = IterativCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
