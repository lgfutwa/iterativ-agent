...
# iterativ modules.  iterativ_cli sets ITERATIV_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("ITERATIV_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ITERATIV_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ITERATIV_TUI_SIDECAR_URL")
...
# tested; override via ``ITERATIV_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ITERATIV_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ITERATIV_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``iterativ --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
