# OpenAI-Compatible API Server for Iterativ Agent
...
SSE streaming. By exposing this endpoint, iterativ-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  iterativ-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.iterativ/.env`
2. Run `iterativ gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with iterativ-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns iterativ-agent as a model) |
...
Authorization: Bearer iterativ-api-key-here
...
But iterativ-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "iterativ-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured iterativ-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of iterativ-agent's core prompt
...
Once implemented, iterativ-agent works as a drop-in backend for:
