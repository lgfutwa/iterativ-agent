...
# 5. Symlinks the 'iterativ' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Iterativ Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/iterativ-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink iterativ into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up iterativ command..."

ITERATIV_BIN="$SCRIPT_DIR/venv/bin/iterativ"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ITERATIV_BIN" "$COMMAND_LINK_DIR/iterativ"
echo -e "${GREEN}✓${NC} Symlinked iterativ → $COMMAND_LINK_DISPLAY_DIR/iterativ"
...
# ============================================================================
# Seed bundled skills into ~/.iterativ/skills/
# ============================================================================

ITERATIV_SKILLS_DIR="${ITERATIV_HOME:-$HOME/.iterativ}/skills"
mkdir -p "$ITERATIV_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.iterativ/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ITERATIV_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  iterativ status        # Check configuration"
if is_termux; then
    echo "  iterativ gateway       # Run gateway in foreground"
else
    echo "  iterativ gateway install # Install gateway service (messaging + cron)"
fi
echo "  iterativ cron list     # View scheduled jobs"
echo "  iterativ doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m iterativ_cli.main setup
fi
