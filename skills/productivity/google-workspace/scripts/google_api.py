#!/usr/bin/env python3
"""Google Workspace API CLI for Iterativ Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Iterativ-facing JSON contract and falls back to the Python client
...
from _iterativ_home import get_iterativ_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ITERATIV_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Iterativ Agent")
