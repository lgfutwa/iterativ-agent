...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ITERATIV_HOME", os.path.expanduser("~/.iterativ")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ITERATIV_HOME", Path.home() / ".iterativ")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Iterativ config paths
# ═══════════════════════════════════════════════════════════════════

ITERATIV_HOME = Path(os.getenv("ITERATIV_HOME", Path.home() / ".iterativ"))
CONFIG_PATH = ITERATIV_HOME / "config.yaml"
PREFILL_PATH = ITERATIV_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Iterativ models — already uncensored, just needs clean prompt
    "iterativ": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "iterativ" in model_lower or "nous" in model_lower:
        return "iterativ"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Iterativ config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.iterativ/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Iterativ for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Iterativ for changes to take effect.")
