...
ITERATIV_HOME_DIR="${ITERATIV_HOME:-$HOME/.iterativ}"
ITERATIV_CFG="${ITERATIV_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Iterativ config has twozero_td MCP entry ──
if [[ ! -f "$ITERATIV_CFG" ]]; then
    echo -e " ${FAIL} Iterativ config not found at ${ITERATIV_CFG}"
    manual_steps+=("Create ${ITERATIV_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ITERATIV_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Iterativ config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Iterativ config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ITERATIV_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ITERATIV_CFG} manually"); }
    manual_steps+=("Restart Iterativ session to pick up config change")
fi
